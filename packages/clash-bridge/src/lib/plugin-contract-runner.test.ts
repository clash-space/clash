...
it("runs declared plugin contracts in the Bridge stdio sandbox with fixture-only broker data", async () => {
  ...
  await writeFile(join(pluginDir, "handler.mjs"), [
    ...
    '        prompt: invocation.input.values.prompt, credentialHandle: message.result.handle,',
    '      } }],',
    '    }) + "\\n");',
    '    return;',
    '  }',
  ...
  await writeFile(join(pluginDir, "contract-tests", "project.json"), JSON.stringify({
    ...
    brokerFixtures: [{
      operation: { kind: "credential.handle", secretId: "provider:fal" },
      response: { status: "ok", result: { handle: "clash-secret://fixture/provider-fal" } },
    }],
    expect: {
      status: "completed",
      outputs: [{
        ...
        value: {
          prompt: "Turn around",
          credentialHandle: "clash-secret://fixture/provider-fal",
        },
      }],
    },
  }));
