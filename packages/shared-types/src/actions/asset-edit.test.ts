...
describe("asset edit action specs", () => {
  it("declares edit capabilities as serializable action specs", () => {
    expect(
      BUILT_IN_ASSET_ACTION_SPECS[ASSET_ACTION_ID.ImageEditor],
    ).toMatchObject({
      family: "edit",
      inputKinds: ["image"],
    ...
    expect(
      BUILT_IN_ASSET_ACTION_SPECS[ASSET_ACTION_ID.VideoClipper].operations,
    ).toEqual([
  ...
  it("derives explicit canvas and implicit preview invocations from the same spec", () => {
    ...
    expect(
      createAssetActionInvocation({ ...base, surface: "canvas" }),
    ).toMatchObject({
    ...
    expect(
      createAssetActionInvocation({ ...base, surface: "asset-preview" }),
    ).toMatchObject({
      actionId: "image-editor",
  ...
  it("resolves operation-specific output kinds from the action spec", () => {
    expect(
      resolveAssetActionOutputKind(ASSET_ACTION_ID.VideoClipper, {
        mode: "screenshot",
        frameTimeSec: 1,
      }),
    ).toBe("image");
