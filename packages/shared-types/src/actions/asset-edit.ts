...
export type AssetActionId = (typeof ASSET_ACTION_ID)[keyof typeof ASSET_ACTION_ID];
...
export const AssetEditActionInvocationSchema = z.discriminatedUnion('actionId', [
...
export type AssetEditActionInvocation = z.infer<typeof AssetEditActionInvocationSchema>;
...
export function legacyEditOriginForSurface(surface: ActionSurface): 'canvas-node' | 'asset-preview' {
  return surface === 'canvas' ? 'canvas-node' : 'asset-preview';
}

export function actionSourceModel(invocation: AssetEditActionInvocation): string {
