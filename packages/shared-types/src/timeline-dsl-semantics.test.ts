...
const GLOBAL_RULE_REACHABILITY_CASES = [
    ...
    },
    {
      ...
      state: {
        tracks: [{
          id: "visual",
          items: [clip("bad-from", "solid", "not a valid expression!", { color: "black" })],
        }],
      },
    },
    {
    ...
    },
    {
      ...
      state: {
        tracks: [{
          id: "effects",
          category: "effect",
          items: [clip("remote", "composition", 0, {
    ...
    },
    {
      ...
      state: {
        tracks: [{
          id: "effects",
          category: "effect",
          items: [clip("react", "composition", 0, {
            compositionKind: "custom",
            runtime: "react",
            compositionId: "react",
    ...
    },
    {
      ...
      state: {
        tracks: [{
          id: "effects",
          category: "effect",
          items: [clip("bad-mg", "composition", 0, {
    ...
    },
    {
      ...
      state: {
        tracks: [{
          ...
          role: "subtitle",
          category: "text",
          items: [clip("caption", "text", 0, { text: "hello", color: "white" })],
        }],
      },
    },
    {
      ...
      state: {
        tracks: [{
          ...
          items: [clip("caption", "text", 0, {
            ...
            wordRefs: [{ id: "word-1", text: "hello", sourceStartFrame: 0, sourceEndFrame: 10 }],
            sourceToOutputMap: [{
              sourceStartFrame: 20,
              sourceEndFrame: 30,
              outputStartFrame: 0,
    ...
    },
    {
      ...
      state: {
        tracks: [{
          id: "overlays",
          items: [clip("derived", "derived-overlay", 0, {
            mediaType: "image",
    ...
    },
    {
      ...
      state: {
        tracks: [{
          id: "effects",
          category: "effect",
          items: [clip("transition", "transition", 15, {
        ...
        tracks: [
          {
            id: "effects",
            category: "effect",
            items: [clip("transition", "transition", 0, {
              durationInFrames: 10,
              transitionType: "crossfade",
              fromItemId: "a",
              toItemId: "b",
        ...
        tracks: [
          ...
          },
          {
            ...
            items: [
              ...
              clip("b", "image", 5, { durationInFrames: 5, src: "assets/b.webp" }),
            ],
          },
        ],
      },
...
function compositionState(sourcePath: string) {
  return {
    tracks: [{
      id: "effects",
      category: "effect",
      items: [clip("composition", "composition", 0, {
        compositionKind: "custom",
        runtime: "html",
...
describe("complete Timeline semantic contract", () => {
  ...
  it("has one explicit evaluator owner for every published global rule", () => {
    const published = semantics.TIMELINE_DSL_GLOBAL_SEMANTIC_RULES.map((rule) => rule.id);
    ...
    expect(Object.values(evaluatorOwners).every((owner) => typeof owner === "function")).toBe(true);
  });

  it("keeps a triggerable regression fixture for every published global rule", () => {
    const published = semantics.TIMELINE_DSL_GLOBAL_SEMANTIC_RULES
      .map((rule) => rule.id)
      .sort();
  ...
  it("accepts transitions between contiguous text clips supported by the renderer", () => {
      ...
      tracks: [
        {
          id: "effects",
          category: "effect",
          items: [clip("transition", "transition", 25, {
            durationInFrames: 10,
            transitionType: "crossfade",
            fromItemId: "title-a",
            toItemId: "title-b",
    ...
    expect(validationRuleIds(state)).not.toContain("timeline.transition.continuity");
  });

  it("publishes every executed global rule to agents", () => {
    const published = new Set(
      (TIMELINE_DSL_DEFINITION.jsonSchema as any)["x-clash-semantic-rules"]
