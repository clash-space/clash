...
function parseProjectAsset(id: string, raw: unknown): ProjectAssetEntry | null {
...
function mutationAuthorityError(doc: LoroDoc): ProjectAssetMutationResult | undefined {
  const version = rawProjectAssetAuthorityVersion(doc);
  if (version === undefined || version === PROJECT_ASSET_AUTHORITY_VERSION) return undefined;
  return { ok: false, error: authorityError(version).error };
}

export function markProjectAssetAuthority(doc: LoroDoc): ProjectAssetAuthorityResult {
  ...
  doc.getMap(PROJECT_ASSET_SCHEMA_CONTAINER)
...
export function readProjectAsset(doc: LoroDoc, id: string): ProjectAssetEntry | null {
...
export function listProjectAssets(doc: LoroDoc): ProjectAssetEntry[] {
  const entries: ProjectAssetEntry[] = [];
  for (const [id, raw] of doc.getMap(PROJECT_ASSETS_CONTAINER).entries()) {
    const entry = parseProjectAsset(id, raw);
...
export function restoreProjectAsset(doc: LoroDoc, id: string): ProjectAssetMutationResult {
