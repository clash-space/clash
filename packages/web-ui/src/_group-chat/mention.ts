 ...
 *
 * Why `#` and not `@`: ChatInput has long used `@` to invoke the image
 * attachment picker, so reusing it here would race with the upload
 * affordance. `#` reads naturally as a channel-style addressee
 * (Slack-ish), and the crew-as-channel framing fits the "invite into
 * room" model.
 *
