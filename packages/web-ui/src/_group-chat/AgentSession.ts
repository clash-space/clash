/**
 * One agent member's WebSocket session + prompt queue.
 *
 * Owns:
 ...
 * constructs AgentSession after the POST /sessions returns a session_id.
 */

import { appendAcpEvent } from '../lib/acpEvents';
import { runtimeWebSocketUrl } from '../lib/runtimeConfig';
import type { AgentView, AgentStatus, QueuedPrompt, Subscriber } from './types';
...
export class AgentSession {
  ...
  availableCommands: AgentView['availableCommands'] = [];
  unread = false;
  ...
  /** Set by GroupChat when this agent becomes / loses focus. Affects
   *  unread bookkeeping for incoming events. */
  isFocused = false;

  constructor(agentMemberId: string, private notify: Subscriber) {
  ...
  /**
   * Enqueue a prompt. If the agent is idle (status === 'connected'),
  ...
  private handleMessage(raw: unknown): void {
    ...
    switch (msg.type) {
      ...
      case 'session.disposed':
        // Agent finished — GroupChat will remove from the panel.
  ...
  /** Cancel every in-flight turn for this agent. (Steer is a separate
