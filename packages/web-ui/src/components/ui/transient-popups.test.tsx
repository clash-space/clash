...
describe("transient popup primitives", () => {
    ...
    it("switches dropdown triggers in one pointer action", () => {
        ...
        fireEvent.pointerDown(screen.getByRole("button", { name: "First trigger" }), { button: 0, ctrlKey: false });
        expect(screen.getByRole("menu", { name: "First trigger" })).toBeTruthy();

        fireEvent.pointerDown(screen.getByRole("button", { name: "Second trigger" }), { button: 0, ctrlKey: false });
        ...
        render(
            <>
                <Popover>
                    ...
                    <PopoverContent role="dialog" aria-label="First panel">First panel</PopoverContent>
                </Popover>
                <Popover>
                    <PopoverTrigger>Second popover</PopoverTrigger>
                    <PopoverContent role="dialog" aria-label="Second panel">Second panel</PopoverContent>
                </Popover>
            </>,
        );
