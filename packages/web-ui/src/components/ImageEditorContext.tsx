...
} from "../features/assets/action-client";
...
export function ImageEditorPanel({
  ...
  const [rotation, setRotation] = useState<0 | 90 | 180 | 270>(
    input.initialParams.rotation ?? 0,
  );
  ...
  const params: ImageEditParams = useMemo(() => {
    const isFull =
      crop.x === 0 &&
      crop.y === 0 &&
      crop.width === input.naturalWidth &&
      crop.height === input.naturalHeight;
...
function DarkenMask({
  ...
  const cw = crop.width * scale,
    ch = crop.height * scale;
  const cls = "absolute bg-slate-950/55 pointer-events-none";
  return (
    <>
      <div
        className={cls}
        style={{ left: 0, top: 0, width: dispW, height: cy }}
      />
      <div
        className={cls}
        style={{
      ...
      <div
        className={cls}
        style={{ left: cx + cw, top: cy, width: dispW - (cx + cw), height: ch }}
      />
    </>
  );
}
...
function CornerHandle({
  ...
  pos: "nw" | "ne" | "sw" | "se";
  dragProps: CropDragBindProps;
}) {
  const cursor =
    pos === "nw" || pos === "se" ? "cursor-nwse-resize" : "cursor-nesw-resize";
  const positionStyle: React.CSSProperties = {
    ...(pos.includes("n") ? { top: -6 } : { bottom: -6 }),
    ...(pos.includes("w") ? { left: -6 } : { right: -6 }),
  };
  const { style: dragStyle, ...restDragProps } =
    dragProps as CropDragBindProps & { style?: React.CSSProperties };
  return (
    <div
      {...restDragProps}
...
function applyDrag(
  ...
  if (kind === "n") {
    ...
    return finalize(
      { x, y: newY, width, height: bottom - newY },
      imgW,
      imgH,
      null,
    );
  ...
  if (kind === "w") {
    ...
    return finalize(
      { x: newX, y, width: right - newX, height },
      imgW,
      imgH,
      null,
    );
...
function expandFromCorner(
  ...
  if (kind === "se")
    return { x: origin.x, y: origin.y, width: newW, height: newH };
  if (kind === "sw")
    return { x: right - newW, y: origin.y, width: newW, height: newH };
  if (kind === "ne")
    return { x: origin.x, y: bottom - newH, width: newW, height: newH };
  /* nw */ return {
    x: right - newW,
    y: bottom - newH,
    width: newW,
