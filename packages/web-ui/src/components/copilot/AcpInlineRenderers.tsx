/** Clash-only inline renderers hosted inside the common Backchat timeline.
...
export type ClashProjectEntityKind =
  "canvas" | "canvas-node" | "timeline" | "director-stage" | "asset";
 ...
 */
function acpToolContentToOutput(
...
function shellCommandFromRaw(rawInput: unknown): string | null {
  ...
  if (Array.isArray(command)) {
    ...
    const shellCommand =
      lcIndex >= 0 ? parts[lcIndex + 1] : parts[parts.length - 1];
...
function isShellLikeTool(tool: AcpToolCallPart): boolean {
  ...
  if (
    isGenericShellToolName(tool.title) ||
    isGenericShellToolName(tool.toolName)
  )
    return true;
  ...
  return (
    ...
    tool.content.some((item) => item.type === "terminal")
  );
}
...
function clashMcpSurface(toolName: string | undefined): ClashMcpSurface {
  ...
  if (
    ...
    return "Canvas";
  if (
    toolName.startsWith("clash_timeline_") ||
    toolName === "clash_cli_timeline"
  )
    return "Timeline";
  if (
    toolName.startsWith("clash_director_") ||
    toolName === "clash_cli_director"
  )
    return "Director";
...
function clashMcpAction(
  toolName: string | undefined,
  surface: ClashMcpSurface,
): string {
  ...
  const surfaceIndex = tokens.findIndex(
    (token) => token === normalizedSurface || `${token}s` === normalizedSurface,
  );
  const actionToken =
    tokens[surfaceIndex >= 0 ? surfaceIndex + 1 : tokens.length - 1] ?? "";
...
function clashMcpResult(tool: AcpToolCallPart): {
  ...
  const contentResult = isRecord(contentOuter.result)
    ? contentOuter.result
    : contentOuter;
...
function clashMcpResultFacts(
  ...
  return Object.entries(structured).flatMap(([key, value]) => {
    if (
      CLASH_MCP_HIDDEN_RESULT_KEYS.has(key) ||
      value === null ||
      value === undefined
    ) {
      return [];
...
function ClashMcpResultDetails({
  ...
  return (
    ...
    >
      {text ? (
        <p className="m-0 whitespace-pre-wrap text-pretty">{text}</p>
      ) : null}
      {facts.length > 0 ? (
        <dl className="m-0 grid grid-cols-[max-content_minmax(0,1fr)] gap-x-3 gap-y-0.5">
          {facts.map((fact) => (
            <div key={`${fact.label}-${fact.value}`} className="contents">
              <dt className="truncate text-neutral-400 dark:text-stone-500">
                {fact.label}
              </dt>
...
function resolveClashEntityLabel(
  ...
  return (
    knownEntities.find((entity) => entity.kind === kind && entity.id === id)
      ?.label ?? entityLabelFromId(id)
  );
}
...
function clashMcpEntities(
  ...
  > = {
    ...
    Director: {
      kind: "director-stage",
      idKeys: ["stageId", "stage_id", "directorStageId", "id"],
    },
  ...
  return items.flatMap((item) => {
    ...
    if (!id) return [];
    const returnedLabel = stringProperty(item, [
      "label",
      "name",
      "title",
      "fileName",
    ...
    return [
      {
        ...
        label:
          returnedLabel ??
          resolveClashEntityLabel(config.kind, id, knownEntities),
      },
    ];
  });
}
...
function ClashProjectEntityResult({
  ...
  return (
    <Button
      ...
      className={cn(
        className,
        "cursor-pointer hover:border-neutral-300 hover:bg-neutral-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/35 dark:hover:border-white/20 dark:hover:bg-white/[0.075]",
      )}
...
function ClashMcpToolRow({
  ...
  const visibleText =
    result.text && !/^\s*[[{]/.test(result.text) ? result.text : undefined;
  const facts = clashMcpResultFacts(result.structured);
  const hasBody =
    entities.length > 0 || Boolean(visibleText) || facts.length > 0;
      ...
      <CollapsibleTrigger asChild>
        <Button
          ...
          className={cn(
            ...
            hasBody
              ? "cursor-pointer focus-visible:underline focus-visible:decoration-neutral-300 focus-visible:underline-offset-4"
              : "cursor-default",
          )}
        >
          <span
          ...
          <span className="flex min-w-0 max-w-full items-baseline gap-2">
            <span
              className={cn(
                "shrink-0 font-medium text-neutral-500 dark:text-stone-300",
                failed && "text-status-down",
              )}
            >
              {action}
          ...
          {failed ? (
            <XCircle className="h-3.5 w-3.5 shrink-0 text-status-down" />
          ) : null}
          {hasBody ? (
...
function parseShellTextOutput(
  text: string,
): Pick<ShellSummary, "stdout" | "stderr" | "exitCode"> {
...
function shellSummaryForTool(
  tool: AcpToolCallPart,
  output: unknown,
): ShellSummary | null {
  if (!isShellLikeTool(tool)) return null;
  const titleCommand = isGenericShellToolName(tool.title)
    ? undefined
    : tool.title;
  const toolNameCommand = isGenericShellToolName(tool.toolName)
    ? undefined
    : tool.toolName;
  const command =
    shellCommandFromRaw(tool.rawInput) ?? titleCommand ?? toolNameCommand;
  if (!command) return null;
  const rawInput = isRecord(tool.rawInput) ? tool.rawInput : {};
  const rawOutput = isRecord(output) ? output : {};
  const parsedTextOutput =
    typeof output === "string" ? parseShellTextOutput(output) : {};
  const stdout =
  ...
  const exitCode =
    ...
    rawOutput.exitCode ??
    rawOutput.code ??
    parsedTextOutput.exitCode;
  return {
    command,
    cwd:
      textField(rawInput.cwd) ??
      textField(rawInput.workdir) ??
      textField(rawInput.workingDirectory),
    stdout,
    stderr,
    exitCode:
...
function ShellToolDetails({
  ...
  return (
    <div
      data-testid="acp-shell-details"
      className="w-full overflow-hidden rounded-xl bg-neutral-100 text-neutral-500 dark:bg-white/10 dark:text-stone-300"
    >
      <div className="px-3 pt-2 text-[13px] font-medium text-neutral-500 dark:text-stone-300">
        Shell
      </div>
      <CodeBlock
...
function AssistantText({
  ...
  return (
    <div
      data-testid="acp-assistant-body"
      data-assistant-section={section}
...
function ToolRow({
  ...
  const claudeCodeMeta = isRecord(tool.meta?.claudeCode)
        ...
        >
          {nonExecutionKind ? (
            ...
            >
              ...
              {userFeedback ? (
                <div className="mt-0.5 text-current/80">{userFeedback}</div>
              ) : null}
            </div>
          ) : null}
              ...
              {tool.locations.map((loc, index) =>
                loc.path ? (
                  <Tooltip label={loc.path} key={`${loc.path}-${index}`}>
                    <span className="font-sans text-[12px] text-neutral-500 dark:text-stone-300">
                      {shortPath(loc.path)}
...
/** Inline surfaces used by the canonical Backchat turn containers. They keep
 * Clash-specific Markdown and tool/MCP behavior without reviving the old
 * flat-message timeline. */
export function AcpAssistantTextInline({
  text,
  section = "answer",
}: {
  text: string;
  ...
  return <AssistantText text={text} section={section} />;
}

export function AcpToolInline({
  tool,
  defaultOpen = false,
  clashEntities = [],
  onOpenClashEntity,
  ...
  return <ToolRow tool={presentedTool} defaultOpen={defaultOpen} />;
}
