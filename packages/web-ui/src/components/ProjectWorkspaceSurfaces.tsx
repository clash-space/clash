...
type ProjectTimelineEditorState = Pick<
    EditorState,
    'tracks' | 'primaryTrackId' | 'compositionWidth' | 'compositionHeight' | 'fps' | 'durationInFrames' | 'assetTranscripts'
...
function ImagePreview({
    ...
    label: string;
    onError: () => void;
    onMetadata: (metadata: { naturalWidth: number; naturalHeight: number }) => void;
}) {
    ...
    return (
        ...
        >
            ...
            <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-lg border border-warm-border bg-warm-surface/95 p-1 shadow-lg backdrop-blur">
                <IconButton label="Zoom out" icon={<MagnifyingGlassMinus className="h-4 w-4" />} size="sm" shape="rounded" onClick={() => setSteppedZoom(-1)} />
                <span className="w-12 text-center text-[11px] font-semibold tabular-nums text-stone-600">{Math.round(zoom * 100)}%</span>
                <IconButton label="Zoom in" icon={<MagnifyingGlassPlus className="h-4 w-4" />} size="sm" shape="rounded" onClick={() => setSteppedZoom(1)} />
                <span className="mx-1 h-5 w-px bg-warm-border" aria-hidden="true" />
                <Button size="sm" shape="rounded" onClick={fitImage} className="h-7 min-h-7 px-2 text-[11px] text-stone-600">Fit image</Button>
                <IconButton label="Actual size" icon={<ArrowsOutSimple className="h-4 w-4" />} size="sm" shape="rounded" onClick={actualSize} />
            </div>
        </div>
    );
}
...
export function ProjectAssetSurface({
    ...
    asset: ProjectAsset;
    renderEditor?: (metadata: ProjectAssetEditMetadata, close: () => void) => ReactNode;
    inspector?: ReactNode;
    headerEndInset?: number;
}) {
    const [failed, setFailed] = useState(false);
    const [mediaMetadata, setMediaMetadata] = useState<ProjectAssetEditMetadata | null>(null);
                    ...
                    {editing && mediaMetadata && renderEditor
                        ...
                        <div className="flex h-full w-full items-center justify-center bg-stone-950 p-6">
                            <video
                                ...
                                playsInline
                                preload="metadata"
                                onLoadedMetadata={(event) => setMediaMetadata({ durationSec: event.currentTarget.duration })}
...
export function ProjectTimelineEditorSurface({
    ...
    const [isProjectAssetDropActive, setIsProjectAssetDropActive] = useState(false);
    const [editorMountReadyTimelineId, setEditorMountReadyTimelineId] = useState<string | null>(null);
    const [editorRevisionKey, setEditorRevisionKey] = useState(timeline.revisionId);
    const [nleAvailability, setNleAvailability] = useState<NleAvailability[] | null>(null);
    const [nleAvailabilityError, setNleAvailabilityError] = useState<string | null>(null);
    ...
    const shouldAnimateInset = previousRightInsetRef.current !== rightInset &&
    ...
    const initialState = (timeline.state && typeof timeline.state === 'object')
        ...
        : undefined;
    const mediaAssetRefs = Array.isArray((timeline.state as { mediaAssetRefs?: unknown } | undefined)?.mediaAssetRefs)
        ? (timeline.state as { mediaAssetRefs: Array<{ assetId: string }> }).mediaAssetRefs
        : undefined;
    const seededEditorAssets = useMemo<EditorAssetInput[]>(() => mediaInputs.map((input) => ({
        id: input.sourceNodeId,
        backingAssetId: input.backingAssetId,
        sourceNodeId: input.sourceNodeId,
        name: input.displayName,
        src: input.src,
        type: input.type,
    })), [mediaInputs]);
    const [editorAssets, setEditorAssets] = useState<EditorAssetInput[]>(seededEditorAssets);
    useEffect(() => {
        let cancelled = false;
        setEditorAssets(seededEditorAssets);
        void Promise.all(mediaInputs.map(async (input): Promise<EditorAssetInput> => {
            try {
                const asset = await getAsset(input.backingAssetId);
                const src = asset.signedUrl || await getSignedUrl(asset.srcR2Key);
                const thumbnail = asset.signedCoverUrl || (asset.coverR2Key
                ...
                return seededEditorAssets.find((asset) => asset.sourceNodeId === input.sourceNodeId)!;
            }
        })).then((resolved) => {
    ...
    }, [mediaInputs, seededEditorAssets]);

    const persistedTimelineState = useCallback((state: EditorState): ProjectTimelineEditorState => ({
            ...
            ...(mediaAssetRefs ? { mediaAssetRefs } : {}),
    }), [mediaAssetRefs, mediaInputs]);
    ...
    const scheduleStatePersist = useCallback((state: EditorState) => {
        if (
    ...
    const refreshNleAvailability = useCallback(async () => {
        ...
        try {
            ...
            setNleAvailabilityError(
                reason instanceof Error ? reason.message : 'Could not check installed editors.',
            );
        }
    }, []);
    ...
    const openInNle = useCallback(async (target: NleTarget) => {
        const desktop = globalThis.__CLASH_DESKTOP__;
        if (!desktop?.openInNle) throw new Error('Open in is available in the Clash desktop app.');
        const state = editorStateRef.current;
        if (!state) throw new Error('Timeline is still loading.');
        const persistedState = persistedTimelineState(state);
        if (!persistCurrentState()) throw new Error('Save the Timeline before opening it in another editor.');
    ...
    const exportTimelineVideo = useCallback(async () => {
        if (!persistCurrentState()) throw new Error('Save the Timeline before exporting it.');
        if (!onExport) throw new Error('The Timeline render backend is unavailable.');
        await onExport(timeline.id);
    }, [onExport, persistCurrentState, timeline.id]);

    const transcribeAsset = useCallback(async (asset: RemotionAsset): Promise<EditorAssetTranscript> => {
        const assetId = asset.backingAssetId ?? asset.id;
        const mediaResponse = await fetch(asset.src);
        if (!mediaResponse.ok) {
            throw new Error(`Could not read ${asset.name} for transcription (HTTP ${mediaResponse.status})`);
        }
        const mediaBlob = await mediaResponse.blob();
        const form = new FormData();
        form.append('file', new File([mediaBlob], asset.name || `${assetId}.${asset.type}`, {
            type: mediaBlob.type || (asset.type === 'video' ? 'video/mp4' : 'audio/wav'),
        }));
        const response = await fetch(runtimeApiUrl('/api/v1/local/audio/transcriptions'), {
            method: 'POST',
            credentials: 'include',
            body: form,
        });
        ...
        if (!response.ok) {
            const message = raw && typeof raw === 'object' && 'error' in raw && typeof raw.error === 'string'
                ? raw.error
    ...
    const handleProjectAssetDragOver = useCallback((event: DragEvent<HTMLElement>) => {
        if (!onProjectAssetDrop || !hasProjectAssetDragData(event.dataTransfer)) return;
        event.preventDefault();
        event.dataTransfer.dropEffect = 'copy';
        setIsProjectAssetDropActive(true);
    }, [onProjectAssetDrop]);
    const handleProjectAssetDrop = useCallback((event: DragEvent<HTMLElement>) => {
        if (!onProjectAssetDrop || !hasProjectAssetDragData(event.dataTransfer)) return;
    ...
    const editorContentRight = rightInset > 0
    ...
    return (
        <main
            ...
            data-project-asset-drop-active={isProjectAssetDropActive ? 'true' : 'false'}
