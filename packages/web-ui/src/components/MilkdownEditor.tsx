...
export type MilkdownFormat = "bold" | "italic" | "heading-2" | "blockquote";
...
const mentionValue = (node: MentionableNode) =>
  `${node.kind ?? "node"}:${node.id}`;
const mentionItemId = (node: MentionableNode) =>
  `milkdown-mention-${mentionValue(node).replace(/[^a-zA-Z0-9_-]/g, "_")}`;
      ...
      view() {
        return {
          update(view: EditorView) {
            const state = mentionPluginKey.getState(
              view.state,
            ) as MentionPluginState;
            if (state?.active) {
...
function AssetMentionMenu({
  ...
  const filtered = useMemo(
    () =>
      nodes.filter((n) => {
        ...
        const searchable =
          `${n.label} ${n.description ?? ""} ${n.type}`.toLowerCase();
    ...
    return definitions.flatMap((definition) => {
      const entries = filtered
        .filter(
          (node) =>
            (node.scope ??
              (node.type === "agent" ? "agents" : "current-canvas")) ===
            definition.scope,
        )
        .sort(
          (left, right) =>
            Number(connectedIds.has(right.id)) -
            Number(connectedIds.has(left.id)),
        );
  ...
  const combobox = useComboboxStore({
    ...
    setSelectedValue(value) {
      ...
      const node = sorted.find(
        (candidate) => mentionValue(candidate) === value,
      );
  ...
  useEffect(() => {
    ...
    onKeyboardHandlerChange((event) =>
      handleMentionComboboxKeyDown(event, {
        store: combobox,
        items: sorted,
  ...
  const typeIcon = (node: MentionableNode) => {
    ...
    if (node.kind === "agent" || node.type === "agent")
      return <Robot className={iconClass} weight="bold" />;
    if (node.kind === "timeline" || node.type === "timeline")
      return <FilmStrip className={iconClass} weight="bold" />;
    if (node.type === "image")
      return <ImageIcon className={iconClass} weight="bold" />;
    if (node.type === "video")
      return <FilmStrip className={iconClass} weight="bold" />;
    if (node.type === "audio")
      return <SpeakerHigh className={iconClass} weight="bold" />;
    if (node.type === "action" || node.type.toLowerCase().includes("action"))
      return <Lightning className={iconClass} weight="fill" />;
    if (node.type === "group")
      return <SquaresFour className={iconClass} weight="bold" />;
...
const MilkdownEditorInner = forwardRef<
...
>(function MilkdownEditorInner(
  ...
  const setMentionKeyHandler = useCallback(
    (handler: ((event: KeyboardEvent) => boolean) | null) => {
          ...
          props: {
            handleKeyDown(view, event) {
              if (
                ...
                const mentionActive = mentionPluginKey.getState(view.state) as
  ...
  const mentionPlugin = useCallback(() => {
    ...
    return createMentionPlugin(
      setMentionState,
      (event) => mentionKeyHandlerRef.current?.(event) ?? false,
    );
  }, [showMentions]);
  ...
  return (
    <>
      <div className="milkdown-editor-wrapper px-3 py-2" onClick={handleClick}>
        <Milkdown />
      </div>
      {showMentions && (
        <AssetMentionMenu
