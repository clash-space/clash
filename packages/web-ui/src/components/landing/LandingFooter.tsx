...
const navigation = {
  ...
  project: [
    ...
    {
      name: "Releases",
      href: "https://github.com/clash-space/clash/releases",
      external: true,
    },
  ],
  legal: [
...
export default function LandingFooter() {
  return (
    <footer
      className="bg-warm-muted/55 border-t border-warm-border relative z-10"
                ...
                <ul role="list" className="mt-6 space-y-4">
                  {navigation.resources.map((item) => (
                    <li key={item.name}>
                      <Link
                        to={item.href}
                        className="text-sm leading-6 text-stone-700 dark:text-stone-300 hover:text-slate-950"
                      >
                        {item.name}
                      </Link>
                    </li>
