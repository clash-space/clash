...
export function useVideoClipper() {
  ...
  if (!ctx)
    throw new Error("useVideoClipper must be used within VideoClipperProvider");
  return ctx;
}
...
export function VideoClipperPanel({
  ...
  const [duration, setDuration] = useState<number>(
    Math.max(0.001, input.durationSec),
  );
...
function RangeReadout({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex flex-col">
      ...
      <span className="font-mono text-base tabular-nums text-content-primary">
        {formatTime(value)}
      </span>
    </div>
  );
...
function Timeline({
  ...
  const clampSec = useCallback(
    (sec: number) => Math.max(0, Math.min(max, sec)),
    [max],
  );
  ...
  const timelineValue =
    mode === "screenshot"
      ...
      : [
          clampSec(Math.min(startSec, endSec)),
          clampSec(Math.max(startSec, endSec)),
        ];
