...
const TextNode = ({ data, selected, id }: NodeProps<Node<Record<string, any>>>) => {
    ...
    useEffect(() => {
        ...
        setContent((prev: string) => (data.content !== undefined && data.content !== prev ? data.content : prev));
    ...
    const normalizedStatus = data.status ? normalizeStatus(data.status as string | undefined) : null;
