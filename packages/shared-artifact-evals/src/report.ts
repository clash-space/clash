...
async function readTrajectoryUsability(
  ...
  for (const path of candidatePaths) {
    ...
    try {
      ...
      const usability = (parsed as { usability?: unknown } | null)?.usability;
      if (usability && typeof usability === "object" && !Array.isArray(usability)) {
        return usability as Record<string, unknown>;
      }
    } catch {
...
function renderUsability(usability: Record<string, unknown> | undefined): string {
  if (!usability) return "";
  ...
  const transports = strings("transportsUsed").map((transport) => transport.toUpperCase());
  const errorCodes = strings("errorCodes");
  const chips = [
    countLabel(count("successfulClashActionCount"), "successful Clash action", "successful Clash actions"),
    countLabel(count("failedClashActionCount"), "failed Clash action", "failed Clash actions"),
    countLabel(count("recoveryCount"), "recovery after failure", "recoveries after failure"),
    countLabel(count("parameterErrorCount"), "parameter error", "parameter errors"),
    countLabel(count("helpActionCount"), "help detour", "help detours"),
    countLabel(count("contractDiscoveryActionCount"), "contract discovery call", "contract discovery calls"),
    ...
    countLabel(count("transportSwitchCount"), "transport switch", "transport switches"),
...
export async function writeSuiteGallery(input: {
  ...
  for (const benchmarkCase of input.report.cases) {
    ...
    const attemptState = [
      benchmarkCase.attempt !== undefined ? `Attempt ${benchmarkCase.attempt}` : undefined,
      benchmarkCase.failure ? `${benchmarkCase.failure.classification} failure` : undefined,
