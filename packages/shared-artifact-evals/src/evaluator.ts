...
import { loadSubmission, type LoadedArtifact, type LoadedSubmission } from "./artifacts";
...
function formatIssues(issues: Array<{ path: PropertyKey[]; message: string }>): string {
  return issues
    .map((issue) => `${issue.path.length > 0 ? issue.path.join(".") : "artifact"}: ${issue.message}`)
    .join("; ");
}
...
function requiredArtifact(
  ...
  if (!artifact) return { ok: false, error: `Submission does not declare artifact '${artifactId}'` };
...
function parseDirector(context: EvaluationContext, artifactId: string): ParsedArtifact<DirectorStageState> {
  const cached = context.directorCache.get(artifactId);
  if (cached) return cached;
  const artifact = requiredArtifact(context, artifactId, "director-stage", true);
  if (!artifact.ok) {
    context.directorCache.set(artifactId, artifact);
    return artifact;
  }
  const json = parseJson(artifact.value.content!, `Director Stage artifact '${artifactId}'`);
  if (!json.ok) {
  ...
  if (!parsed.success) {
    result = { ok: false, error: `Invalid Director Stage artifact: ${formatIssues(parsed.error.issues)}` };
  } else {
    const semanticIssues = directorStageSemanticIssues(parsed.data);
    result = semanticIssues.length === 0
      ? { ok: true, value: parsed.data }
      : { ok: false, error: `Invalid Director Stage references: ${semanticIssues.join("; ")}` };
  }
...
function directorStageSemanticIssues(stage: DirectorStageState): string[] {
  ...
  const registerIds = (items: Array<{ id: string }>, label: string): void => {
    for (const item of items) {
      ...
      if (prior) issues.push(`duplicate id '${item.id}' in ${prior} and ${label}`);
  ...
  const sequenceShotIds = new Set((stage.shotSequence ?? []).map((shot) => shot.id));
  const actionClipIds = new Set((stage.animation?.actionClips ?? []).map((clip) => clip.id));
  const storyBeatIds = new Set((stage.animation?.storyBeats ?? []).map((beat) => beat.id));
  const motionAssetIds = new Set((stage.motionAssets ?? []).map((asset) => asset.id));
  ...
  for (const object of stage.objects) {
    if (object.attachment && !objectIds.has(object.attachment.parentId)) {
      issues.push(`object '${object.id}' attachment parent '${object.attachment.parentId}' does not resolve`);
    }
  ...
  const attachmentParents = new Map(
    stage.objects.flatMap((object) => object.attachment
  ...
  const visitAttachment = (objectId: string, chain: string[]): void => {
    ...
    if (parentId && objectIds.has(parentId)) visitAttachment(parentId, [...chain, objectId]);
    attachmentState.set(objectId, "visited");
  };
  for (const objectId of attachmentParents.keys()) visitAttachment(objectId, []);
  for (const camera of stage.cameras) {
    for (const targetId of [camera.targetObjectId, ...(camera.targetObjectIds ?? [])]) {
      if (targetId && !objectIds.has(targetId)) {
        issues.push(`camera '${camera.id}' target '${targetId}' does not resolve`);
      }
    }
  }
  for (const shot of stage.shots) {
    if (!cameraIds.has(shot.cameraId)) issues.push(`shot '${shot.id}' camera '${shot.cameraId}' does not resolve`);
    if (shot.sequenceShotId && !sequenceShotIds.has(shot.sequenceShotId)) {
      issues.push(`shot '${shot.id}' sequenceShotId '${shot.sequenceShotId}' does not resolve`);
    }
    if (
      ...
      issues.push(`shot '${shot.id}' time ${shot.timeSeconds}s exceeds stage duration ${stage.animation.durationSeconds}s`);
    }
  }
  const sequence = stage.shotSequence ?? [];
  for (let index = 0; index < sequence.length; index += 1) {
    const shot = sequence[index]!;
    if (!cameraIds.has(shot.cameraId)) issues.push(`sequence shot '${shot.id}' camera '${shot.cameraId}' does not resolve`);
    if (stage.animation && shot.startTime + shot.durationSeconds > stage.animation.durationSeconds) {
      issues.push(`sequence shot '${shot.id}' exceeds stage duration ${stage.animation.durationSeconds}s`);
    }
    const previous = sequence[index - 1];
    if (previous && shot.startTime < previous.startTime + previous.durationSeconds) {
      issues.push(`sequence shot '${shot.id}' overlaps or precedes '${previous.id}'`);
    }
    for (const id of shot.actionClipIds ?? []) {
      if (!actionClipIds.has(id)) issues.push(`sequence shot '${shot.id}' action clip '${id}' does not resolve`);
    }
    for (const id of shot.storyBeatIds ?? []) {
      if (!storyBeatIds.has(id)) issues.push(`sequence shot '${shot.id}' story beat '${id}' does not resolve`);
    }
    const subjectIds = [
    ...
    for (const id of subjectIds) {
      if (id && !objectIds.has(id)) issues.push(`sequence shot '${shot.id}' subject '${id}' does not resolve`);
    }
    if (shot.cameraMove?.rig?.orientation.mode === "target-object") {
      const id = shot.cameraMove.rig.orientation.objectId;
      if (!objectIds.has(id)) issues.push(`sequence shot '${shot.id}' rig target '${id}' does not resolve`);
    }
  }

  if (stage.animation) {
    for (const track of stage.animation.tracks) {
      if (!targetIds.has(track.targetId)) issues.push(`animation track '${track.id}' target '${track.targetId}' does not resolve`);
      const keyframeIds = new Set<string>();
      const keyframeTimes = new Set<number>();
      for (const keyframe of track.keyframes) {
        if (keyframeIds.has(keyframe.id)) issues.push(`animation track '${track.id}' has duplicate keyframe id '${keyframe.id}'`);
        if (keyframeTimes.has(keyframe.time)) issues.push(`animation track '${track.id}' has duplicate keyframe time ${keyframe.time}`);
        if (keyframe.time > stage.animation.durationSeconds) {
          issues.push(`animation track '${track.id}' keyframe ${keyframe.time}s exceeds stage duration`);
        }
    ...
    for (const clip of stage.animation.actionClips ?? []) {
      if (!objectIds.has(clip.targetId)) issues.push(`action clip '${clip.id}' target '${clip.targetId}' does not resolve`);
      if (clip.motionAssetId && !motionAssetIds.has(clip.motionAssetId)) {
        issues.push(`action clip '${clip.id}' motion asset '${clip.motionAssetId}' does not resolve`);
      }
      if (clip.startTime + clip.durationSeconds > stage.animation.durationSeconds) {
    ...
    for (const beat of stage.animation.storyBeats ?? []) {
      for (const id of beat.participantIds) {
        if (!objectIds.has(id)) issues.push(`story beat '${beat.id}' participant '${id}' does not resolve`);
      }
      if (beat.dialogue && !objectIds.has(beat.dialogue.speakerId)) {
        issues.push(`story beat '${beat.id}' speaker '${beat.dialogue.speakerId}' does not resolve`);
      }
      if (beat.startTime + beat.durationSeconds > stage.animation.durationSeconds) {
        issues.push(`story beat '${beat.id}' exceeds stage duration`);
      }
    }
    for (const cue of stage.animation.cameraCues ?? []) {
      if (!cameraIds.has(cue.cameraId)) issues.push(`camera cue '${cue.id}' camera '${cue.cameraId}' does not resolve`);
      if (cue.startTime + cue.durationSeconds > stage.animation.durationSeconds) {
...
function parseTimeline(context: EvaluationContext, artifactId: string): ParsedArtifact<TimelineData> {
...
function parseRemotionComponent(
  ...
  if (cached) return cached;
  const artifact = requiredArtifact(context, artifactId, "remotion-component", true);
  if (!artifact.ok) {
  ...
  if (!/\bfrom\s+["']remotion["']/u.test(source)) {
    failures.push("the component must import its animation primitives from remotion");
  }
  const importSpecifiers = [...source.matchAll(/\bfrom\s+["']([^"']+)["']/gu)]
    .map((match) => match[1]!)
    .filter((specifier) => specifier !== "react" && specifier !== "remotion");
  if (importSpecifiers.length > 0) {
    failures.push(`unsupported imports: ${[...new Set(importSpecifiers)].join(", ")}`);
  }
  if (/\bimport\s*\(/u.test(source) || /\brequire\s*\(/u.test(source)) {
    failures.push("dynamic import and require are not supported by the inline Remotion runtime");
  }
  const bodyParts = [...source.matchAll(/data-character-part\s*=\s*["']([^"']+)["']/gu)]
    .map((match) => canonicalBodyPartId(match[1]!));
  const result: ParsedArtifact<RemotionComponentSource> = failures.length === 0
    ? { ok: true, value: { source, bytes: artifact.value.content!.byteLength, bodyParts } }
    : { ok: false, error: `Invalid Remotion component artifact: ${failures.join("; ")}` };
...
function evaluateArtifactExists(
  ...
  const bytes = artifact.value.evidence?.bytes ?? artifact.value.content?.byteLength ?? 0;
...
function evaluateArtifactSet(
  ...
  const matching = context.submission.artifacts.filter((artifact) => (
    (!rubric.kind || artifact.descriptor.kind === rubric.kind)
    && !artifact.error
    && (artifact.evidence?.bytes ?? 0) >= (rubric.minBytes ?? 0)
  ));
  const rejected = context.submission.artifacts.filter((artifact) => (
    (!rubric.kind || artifact.descriptor.kind === rubric.kind) && artifact.error
  ));
...
function evaluateDirectorStage(
  ...
  const animatedTracks = tracks.filter((track) => (
    ...
    && new Set(track.keyframes.map((keyframe) => JSON.stringify(keyframe.value))).size >= 2
  )).length;
  const actionClips = stage.animation?.actionClips ?? [];
  const actions = new Set<string>(actionClips.map((clip) => clip.action));
  const mannequins = stage.objects.filter((object) => object.kind === "mannequin").length;
  const mannequin = mannequins > 0;
  const failures: string[] = [];
  if (stage.objects.length < (rubric.minObjects ?? 0)) failures.push(`objects ${stage.objects.length}/${rubric.minObjects}`);
  if (stage.cameras.length < (rubric.minCameras ?? 0)) failures.push(`cameras ${stage.cameras.length}/${rubric.minCameras}`);
  if (stage.shots.length < (rubric.minCapturedShots ?? 0)) {
    failures.push(`captured shots ${stage.shots.length}/${rubric.minCapturedShots}`);
  }
  ...
  if (animatedTracks < (rubric.minAnimatedTracks ?? 0)) failures.push(`animated tracks ${animatedTracks}/${rubric.minAnimatedTracks}`);
  if (actionClips.length < (rubric.minActionClips ?? 0)) failures.push(`action clips ${actionClips.length}/${rubric.minActionClips}`);
  if (mannequins < (rubric.minMannequins ?? 0)) failures.push(`mannequins ${mannequins}/${rubric.minMannequins}`);
  if (rubric.requireMannequin && !mannequin) failures.push("mannequin missing");
  const missingActions = (rubric.requiredActions ?? []).filter((action) => !actions.has(action));
  if (missingActions.length > 0) failures.push(`missing actions: ${missingActions.join(", ")}`);
  ...
  return checkResult(
    rubric,
    failures.length === 0,
    failures.length === 0 ? "Director Stage requirements satisfied" : failures.join("; "),
    metrics,
  );
}

function timelineItems(timeline: TimelineData): TimelineData["tracks"][number]["items"] {
  return timeline.tracks.flatMap((track) => Array.isArray(track.items) ? track.items : []);
}
...
function evaluateTimeline(
  ...
  const missingTypes = (rubric.requiredItemTypes ?? []).filter((type) => !itemTypes.has(type));
  const failures: string[] = [];
  if (parsed.value.tracks.length < (rubric.minTracks ?? 0)) failures.push(`tracks ${parsed.value.tracks.length}/${rubric.minTracks}`);
  if (items.length < (rubric.minItems ?? 0)) failures.push(`items ${items.length}/${rubric.minItems}`);
  const durationInFrames = typeof parsed.value.durationInFrames === "number"
    ? parsed.value.durationInFrames
    : 0;
  if (durationInFrames < (rubric.minDurationInFrames ?? 0)) {
    failures.push(`duration ${durationInFrames}/${rubric.minDurationInFrames} frames`);
  }
  if (missingTypes.length > 0) failures.push(`missing item types: ${missingTypes.join(", ")}`);
  return checkResult(
    rubric,
    failures.length === 0,
    failures.length === 0
...
async function decodeVisualFrame(
  ...
  const ffprobe = process.env.CLASH_FFPROBE_PATH ?? process.env.FFPROBE_PATH ?? "ffprobe";
  const ffmpeg = process.env.CLASH_FFMPEG_PATH ?? process.env.FFMPEG_PATH ?? "ffmpeg";
  try {
    ...
    const stream = parsed.streams?.find((candidate) => (
      typeof candidate.width === "number" && typeof candidate.height === "number"
    ));
    if (!stream?.width || !stream.height) {
      return { ok: false, error: `Image artifact '${artifactId}' has no decodable dimensions` };
    }
    ...
    const decoded = await execFileAsync(ffmpeg, [
      ...
      "-i", path,
      "-frames:v", "1",
      "-vf", `scale=${sampleWidth}:${sampleHeight}:flags=neighbor`,
      "-f", "rawvideo",
      "-pix_fmt", "rgb24",
      "pipe:1",
    ], {
    ...
    if (rgb.byteLength !== sampleWidth * sampleHeight * 3) {
      return { ok: false, error: `Image artifact '${artifactId}' did not decode to one RGB frame` };
    }
    return {
      ok: true,
      value: {
...
function foregroundEdgeRatio(
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (color.some((value, channel) => Math.abs(value - background[channel]!) > backgroundTolerance)) {
        foregroundPixels += 1;
      }
    }
  }
...
async function evaluateVisualFrames(
  ...
  for (const artifactId of rubric.artifactIds) {
    ...
    if (!artifact.ok || !artifact.value.absolutePath) {
      failures.push(artifact.ok ? `Image '${artifactId}' has no safe local path` : artifact.error);
      continue;
    }
    const frame = await decodeVisualFrame(artifactId, artifact.value.absolutePath);
    if (!frame.ok) {
      failures.push(frame.error);
      continue;
    }
    if (frame.value.width !== rubric.width || frame.value.height !== rubric.height) {
      failures.push(`Image '${artifactId}' dimensions ${frame.value.width}x${frame.value.height}/${rubric.width}x${rubric.height}`);
    }
  ...
  for (let left = 0; left < decoded.length; left += 1) {
    for (let right = left + 1; right < decoded.length; right += 1) {
      pairDifferences.push(frameMeanAbsoluteDifference(decoded[left]!.rgb, decoded[right]!.rgb));
    }
  }
  const distinctPairs = pairDifferences.filter((difference) => (
    difference >= rubric.minMeanAbsoluteDifference
  )).length;
  if (distinctPairs < rubric.minDistinctPairs) {
    failures.push(`distinct frame pairs ${distinctPairs}/${rubric.minDistinctPairs}`);
  }

  const edgeRatios = rubric.safeArea
    ? decoded.map((frame) => foregroundEdgeRatio(
        frame,
    ...
    : [];
  if (rubric.safeArea) {
    const clipped = decoded.filter((_frame, index) => (
      edgeRatios[index]! > rubric.safeArea!.maxForegroundEdgeRatio
    ));
    if (clipped.length > 0) {
      failures.push(`safe-area edge contains foreground in: ${clipped.map((frame) => frame.artifactId).join(", ")}`);
    }
  }
  ...
  return checkResult(
    rubric,
    failures.length === 0,
    failures.length === 0 ? "Visual frame evidence is decoded, distinct, and safely composed" : failures.join("; "),
    {
      decodedFrames: decoded.length,
      distinctPairs,
      pairDifferences: pairDifferences.map((value) => value.toFixed(5)),
      maxForegroundEdgeRatio: edgeRatios.length > 0 ? Math.max(...edgeRatios) : 0,
    },
  );
}
...
async function evaluateMedia(
  ...
  if (!artifact.value.absolutePath) {
    return checkResult(rubric, false, `Artifact '${rubric.artifactId}' has no safe local path`);
  }
  ...
  if (rubric.minDurationSeconds !== undefined && durationSeconds < rubric.minDurationSeconds) {
    failures.push(`duration ${durationSeconds}s is below ${rubric.minDurationSeconds}s`);
  }
  if (rubric.maxDurationSeconds !== undefined && durationSeconds > rubric.maxDurationSeconds) {
    failures.push(`duration ${durationSeconds}s exceeds ${rubric.maxDurationSeconds}s`);
  }
  return checkResult(
    rubric,
    failures.length === 0,
    failures.length === 0 ? "Media requirements satisfied" : failures.join("; "),
    {
      width: video?.width ?? 0,
...
function evaluateMgCharacter(
  ...
  const missingBodyParts = (rubric.requiredBodyParts ?? []).filter((id) => (
    !layerIds.has(canonicalBodyPartId(id))
  ));
  const missingApis = (rubric.requiredRemotionApis ?? []).filter((api) => (
    !new RegExp(`\\b${api.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}\\b`, "u").test(component.source)
  ));
  const failures: string[] = [];
  if (component.bytes < (rubric.minSourceBytes ?? 0)) {
    failures.push(`source bytes ${component.bytes}/${rubric.minSourceBytes}`);
  }
  if (missingBodyParts.length > 0) failures.push(`missing body parts: ${missingBodyParts.join(", ")}`);
  if (missingApis.length > 0) failures.push(`missing Remotion APIs: ${missingApis.join(", ")}`);
  return checkResult(
    rubric,
    failures.length === 0,
    failures.length === 0 ? "Remotion MG character source requirements satisfied" : failures.join("; "),
    {
...
function canonicalBodyPartId(value: string): string {
  ...
  const part = ["arm", "leg", "head", "torso"].find((candidate) => tokens.includes(candidate));
  if (!part) return tokens.join("-");
  if (part === "head" || part === "torso") return part;
  const side = ["left", "right"].find((candidate) => tokens.includes(candidate));
  return side ? `${part}-${side}` : part;
}

function collectStringField(items: Array<Record<string, unknown>>, fields: string[]): Set<string> {
  const values = new Set<string>();
  for (const item of items) {
    for (const field of fields) {
      if (typeof item[field] === "string" && item[field]) values.add(item[field]);
    }
  }
  return values;
}
...
function evaluateMixedLineage(
  ...
  const directorAssetIds = new Set(director.value.shots.map((shot) => shot.assetId));
  const linkedDirectorAssets = [...directorAssetIds].filter((id) => timelineAssetIds.has(id));
  const remotionItems = items.filter((item) => (
  ...
  const failures: string[] = [];
  if (linkedDirectorAssets.length === 0) failures.push("no Director shot asset is referenced by the Timeline");
  if (!componentLinked) failures.push("Timeline does not contain a live Remotion sourceNodeId reference");
  return checkResult(
    rubric,
    failures.length === 0,
    failures.length === 0 ? "Director, Timeline, and Remotion MG lineage is connected" : failures.join("; "),
    { directorLinks: linkedDirectorAssets.length, componentLinked, remotionItems: remotionItems.length },
  );
}

async function evaluateRubric(rubric: ArtifactRubric, context: EvaluationContext): Promise<EvaluationCheck> {
  switch (rubric.type) {
    case "artifact-exists": return evaluateArtifactExists(rubric, context);
    case "artifact-set": return evaluateArtifactSet(rubric, context);
    case "director-stage": return evaluateDirectorStage(rubric, context);
    case "timeline": return evaluateTimeline(rubric, context);
    case "mg-character": return evaluateMgCharacter(rubric, context);
    case "media": return evaluateMedia(rubric, context);
    case "visual-frames": return evaluateVisualFrames(rubric, context);
    case "mixed-lineage": return evaluateMixedLineage(rubric, context);
  }
}

function failedChecks(benchmark: ArtifactBenchmarkCase, detail: string): EvaluationCheck[] {
...
function evaluateOutcomeGate(
  ...
  const byId = new Map(submission.artifacts.map((artifact) => [artifact.descriptor.id, artifact]));
  ...
  for (const deliverable of benchmark.outcome.deliverables) {
    ...
    if (artifact.descriptor.kind !== deliverable.kind || artifact.error || !artifact.evidence) {
      invalidArtifactIds.push(deliverable.artifactId);
    }
  }
  const passed = missingArtifactIds.length === 0 && invalidArtifactIds.length === 0;
  const details: string[] = [];
  if (missingArtifactIds.length > 0) details.push(`missing deliverables: ${missingArtifactIds.join(", ")}`);
  if (invalidArtifactIds.length > 0) details.push(`invalid deliverables: ${invalidArtifactIds.join(", ")}`);
  return {
    status: passed ? "pass" : "fail",
    detail: passed ? "All outcome deliverables are declared, readable, and have the required kind" : details.join("; "),
    missingArtifactIds,
    invalidArtifactIds,
  };
}
...
export async function evaluateSubmission(
  ...
  const benchmarkResult = ArtifactBenchmarkCaseSchema.safeParse(input.benchmark);
  ...
  if (!submission.submission) {
    return {
      ...
      status: "fail",
      score: 0,
      checks: failedChecks(benchmark, submission.error ?? "Submission could not be loaded"),
      artifacts: [],
      outcomeGate: {
        ...unavailableOutcomeGate,
        missingArtifactIds: benchmark.outcome.deliverables.map((deliverable) => deliverable.artifactId),
      },
      error: submission.error,
    };
  }

  const evidence = submission.artifacts.flatMap((artifact) => artifact.evidence ? [artifact.evidence] : []);
  ...
  const context: EvaluationContext = {
    submission,
    artifactById: new Map(submission.artifacts.map((artifact) => [artifact.descriptor.id, artifact])),
    directorCache: new Map(),
    timelineCache: new Map(),
    remotionComponentCache: new Map(),
  };
  const checks = await Promise.all(benchmark.rubric.map((rubric) => evaluateRubric(rubric, context)));
  const totalWeight = checks.reduce((sum, check) => sum + check.weight, 0);
  const awardedWeight = checks.reduce((sum, check) => sum + check.awardedWeight, 0);
  const score = totalWeight > 0 ? roundedScore((awardedWeight / totalWeight) * 100) : 0;
  const requiredGatePassed = checks.every((check) => !check.required || check.status === "pass");
  const outcomeGate = evaluateOutcomeGate(benchmark, submission);
  return {
    schemaVersion: 1,
    benchmarkId: benchmark.id,
    taskId: submission.submission.taskId,
    status: requiredGatePassed && outcomeGate.status === "pass" && score >= benchmark.passScore ? "pass" : "fail",
    score,
    checks,
    artifacts: evidence,
    outcomeGate,
