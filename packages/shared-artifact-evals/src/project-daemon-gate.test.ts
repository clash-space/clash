...
describe("benchmark project daemon gate", () => {
  it("classifies daemon setup failure as infrastructure and never launches Codex", async () => {
    const root = await mkdtemp(join(tmpdir(), "clash-daemon-gate-"));
    ...
    await writeFile(
      ...
      [
        ...
        'if (argv.join(" ") === "canvas connect") process.exit(52)',
        'process.exit(2)',
      ].join("\n"),
      "utf8",
    );
    ...
    const report = await runBenchmarkSuite({
      suite: {
        schemaVersion: 1,
        id: "daemon-gate-suite",
        title: "Daemon gate suite",
        cases: [
          {
            id: "daemon-gate",
            title: "Daemon gate",
    ...
    expect(report.cases[0]).toMatchObject({
      ...
      failure: {
        ...
        phase: "project-daemon-setup",
      },
    });
    expect(report.cases[0]?.failure?.detail).toMatch(
      /project host exited before readiness with code 52/i,
    );
    await expect(lstat(agentStartedPath)).rejects.toMatchObject({
      code: "ENOENT",
    });
    const caseRoot = join(outputRoot, "daemon-gate-run", "daemon-gate");
    expect(
      JSON.parse(await readFile(join(caseRoot, "clash-project-host.json"), "utf8")),
