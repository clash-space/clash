...
import { editorReducer, editorInitialState, normalizeInitialState } from './EditorContext';
...
describe('main storyline invariant', () => {
  it('keeps an unlabeled primary lane present on an empty timeline', () => {
    const normalized = normalizeInitialState({ tracks: [], primaryTrackId: null });
  ...
  it('replaces the primary lane immediately when it is the only track removed', () => {
    const state = seedState([
      { ...makeTrack('story'), category: 'primary', role: 'primary-video' } as Track,
...
describe('subtitle text sticker normalization', () => {
  it('normalizes a legacy continuous caption item into editable sentence stickers', () => {
    const normalized = normalizeInitialState({
      tracks: [{
        ...
        items: [{
          ...
          wordRefs: [
            { id: 'word-1', text: 'First sentence.', sourceStartFrame: 6, sourceEndFrame: 30 },
            { id: 'word-2', text: 'Second sentence.', sourceStartFrame: 45, sourceEndFrame: 75 },
          ],
          sourceToOutputMap: [
            { sourceStartFrame: 6, sourceEndFrame: 30, outputStartFrame: 6, outputEndFrame: 30 },
            { sourceStartFrame: 6, sourceEndFrame: 30, outputStartFrame: 6, outputEndFrame: 30 },
            { sourceStartFrame: 45, sourceEndFrame: 75, outputStartFrame: 45, outputEndFrame: 75 },
          ],
        }],
      }],
      primaryTrackId: null,
    ...
    const stickers = normalized.tracks.find((track) => track.id === 'text')?.items;
    ...
    expect(stickers).toMatchObject([
      {
        ...
        from: 6,
        durationInFrames: 24,
        cues: [{ startFrame: 0, durationInFrames: 24, text: 'First sentence.' }],
        wordRefs: [{ id: 'word-1' }],
      },
      {
        ...
        from: 45,
        durationInFrames: 30,
        cues: [{ startFrame: 0, durationInFrames: 30, text: 'Second sentence.' }],
        wordRefs: [{ id: 'word-2' }],
      },
    ]);
    expect(stickers?.map((item) => item.type === 'text' ? item.sourceToOutputMap : undefined)).toEqual([
      [{
        sourceStartFrame: 6,
        sourceEndFrame: 30,
        outputStartFrame: 0,
  ...
  it('keeps sticker duration, cue duration, and edited text synchronized', () => {
    ...
    const sticker = resized.tracks.find((track) => track.id === 'text')?.items[0] as any;
...
describe('editorReducer — item ops', () => {
  it('keeps the persistent primary lane visual-only for video and image media', () => {
    const state = seedState([
      { ...makeTrack('primary'), category: 'primary', role: 'primary-video' } as Track,
    ...
    expect(withVideo.tracks[0].items.map((item) => item.type)).toEqual(['video']);
  });
  ...
  it('moves items atomically only between compatible track categories', () => {
    ...
    const rejected = editorReducer(state, {
      type: 'MOVE_ITEM',
      payload: { sourceTrackId: 'visual-a', targetTrackId: 'text', itemId: 'v1', from: 10 },
    } as any);
    expect(rejected).toBe(state);

    const moved = editorReducer(state, {
      type: 'MOVE_ITEM',
      payload: { sourceTrackId: 'visual-a', targetTrackId: 'visual-b', itemId: 'v1', from: 10 },
    } as any);
    expect(moved.tracks.find((track) => track.id === 'visual-a')).toBeUndefined();
    expect(moved.tracks.find((track) => track.id === 'visual-b')?.items[0]).toMatchObject({
      id: 'v1',
      from: 10,
    });
  });
  ...
  it('RIPPLE_DELETE_RANGE removes a spoken range, splits its clip, and closes the gap', () => {
    ...
    expect(next.tracks[0].items).toMatchObject([
      ...
      { id: 'clip-ripple-30-45', from: 30, durationInFrames: 45, sourceStartInFrames: 55 },
  ...
  it('RIPPLE_DELETE_RANGE slices keyframes around a removed middle range', () => {
    ...
    const state = seedState([makeTrack('dialogue', [animated])], { durationInFrames: 90 });
  ...
  it('RIPPLE_DELETE_RANGE rebinds a later transition to the clip segment that still reaches its seam', () => {
    ...
    expect(next.tracks.find((track) => track.id === 'media')?.items).toMatchObject([
      { id: 'a', from: 0, durationInFrames: 53 },
      { id: 'a-ripple-53-55', from: 53, durationInFrames: 25 },
      { id: 'b', from: 78, durationInFrames: 120 },
    ]);
    expect(next.tracks.find((track) => track.id === 'transitions')?.items).toMatchObject([
      {
        id: 'tx-a-b',
        from: 66,
        durationInFrames: 24,
  ...
  it('RIPPLE_DELETE_RANGE removes deleted words from structured captions without duplicating the caption item', () => {
    const caption = {
      ...
      sourceToOutputMap: [
        { sourceStartFrame: 0, sourceEndFrame: 90, outputStartFrame: 0, outputEndFrame: 90 },
      ],
      cues: [{
        id: 'cue-1',
        startFrame: 0,
    ...
    const captions = next.tracks.find((track) => track.id === 'subtitles')!.items;
  ...
  it('RESTORE_TIMELINE_SNAPSHOT gives transcript editing a non-destructive undo', () => {
    const originalTracks = [makeTrack('dialogue', [makeVideo('clip', 0, 90, 10)])];
    const edited = seedState([makeTrack('dialogue', [makeVideo('clip', 0, 30, 10)])], {
      durationInFrames: 75,
    });
...
describe('editorReducer — keyframed trim', () => {
  ...
  it('rebases keys when trimming the start and leaves item-local keys unchanged when moving', () => {
    ...
    expect((moved.tracks[0].items[0] as VideoItem).keyframes).toEqual(animatedItem().keyframes);
  });
});
...
describe('editorReducer — SPLIT_ITEM', () => {
  ...
  it('slices and rebases item-local keyframes on both split pieces', () => {
    ...
    const next = editorReducer(
      seedState([makeTrack('t', [item])]),
      { type: 'SPLIT_ITEM', payload: { trackId: 't', itemId: 'animated', splitFrame: 130 } },
    );
  ...
  it('splits an editable subtitle Text sticker with relative cue timing on both pieces', () => {
    ...
    const s = editorReducer(
      seedState([makeTrack('text', [item])]),
      { type: 'SPLIT_ITEM', payload: { trackId: 'text', itemId: 'sentence', splitFrame: 35 } },
    );
    const items = s.tracks[0].items as SubtitleTextItem[];

    expect(items).toHaveLength(2);
    expect(items.map((piece) => ({
      from: piece.from,
...
describe('editorReducer — selection / playback / scalars', () => {
  ...
  it('SET_ASSET_TRANSCRIPT keeps linked subtitle Text word refs, cues, and top-level text in sync', () => {
    ...
    const caption = {
      ...
      sourceToOutputMap: [
        { sourceStartFrame: 0, sourceEndFrame: 18, outputStartFrame: 0, outputEndFrame: 18 },
      ],
      cues: [{
        id: 'cue',
        startFrame: 0,
    ...
    const textItem = next.tracks.find((track) => track.id === 'subtitles')!.items[0] as any;
