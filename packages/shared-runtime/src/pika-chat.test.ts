...
describe("Pika synchronous chat", () => {
  it.each([
    ...
    [
      ...
      "/genai/v1beta/models/gemini-3.6-flash:generateContent",
      { candidates: [{ content: { parts: [{ text: "gemini" }] } }] },
      "gemini",
    ],
  ])(
    "routes %s through its documented Pika protocol",
    async (model, path, body, text) => {
