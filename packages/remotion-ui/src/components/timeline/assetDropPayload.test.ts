...
function transfer(values: Record<string, string>): Pick<DataTransfer, 'getData'> {
  return {
...
describe('resolveAssetDropPayload', () => {
  it('uses an asset already available in the Timeline scope', () => {
    ...
    expect(resolveAssetDropPayload({
  ...
  it('ignores malformed or mismatched serialized payloads', () => {
    ...
    expect(resolveAssetDropPayload({
      assetId: 'project-ref-1',
      dataTransfer: transfer({ asset: JSON.stringify({ id: 'other', type: 'image' }) }),
      assets: [],
    })).toBeUndefined();
  });
});
