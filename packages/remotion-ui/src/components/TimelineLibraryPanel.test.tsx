...
describe("TimelineLibraryPanel", () => {
  ...
  it("searches the catalog and applies a real preset to editor state", async () => {
    ...
    expect(
      document.querySelectorAll('[data-library-card="true"]'),
    ).toHaveLength(1);
    ...
    expect(latest!.tracks.some((track) => track.category === "primary")).toBe(
      true,
    );
  });

  it("admits catalog media before creating a Timeline item", async () => {
    let latest: EditorState | null = null;
    const admit = vi.fn(async (input) => ({
      ...input,
      projectAssetId: "project-sound-click",
      src: "http://127.0.0.1/assets/project-sound-click",
    }));
    render(
      <EditorProvider>
        <TimelineLibraryPanel onAdmitLibraryMedia={admit} />
        <StateProbe
          onState={(state) => {
            latest = state;
          }}
    ...
    fireEvent.change(screen.getByLabelText("Search creative assets"), {
      target: { value: "Mouse Click" },
    });
    fireEvent.click(screen.getByLabelText("Apply Mouse Click"));
  ...
  it("accepts the category selected by the editor top-level tool rail", () => {
    ...
    expect(screen.getByLabelText("Filter FX").className).toContain(
      "text-brand",
    );
  ...
  it("surfaces an invalid transition edit point through the Timeline notice", () => {
    ...
    expect(
      container.querySelector("[data-timeline-notice]")?.textContent,
