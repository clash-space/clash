...
const registry =
  (
    components as unknown as {
      TIMELINE_RENDER_FIELD_CONSUMERS?: ConsumerRegistry;
    }
...
describe("renderer Timeline field consumer gate", () => {
  ...
  it("records root and track renderer behavior instead of treating persistence as output", () => {
    ...
    expect(rootTrackRegistry.root?.primaryTrackId?.consumers).toContain(
      "unsupported",
    );
    expect(rootTrackRegistry.root?.assetTranscripts?.consumers).toContain(
      "persistence",
    );
    expect(rootTrackRegistry.track?.role?.consumers).toContain("rendered");
    expect(rootTrackRegistry.track?.category?.consumers).toContain(
      "unsupported",
    );
  ...
  it("classifies every base and variant field for every item type", () => {
    for (const itemType of TIMELINE_DSL_ITEM_TYPES) {
      ...
      expect(Object.keys(registry[itemType] ?? {}).sort(), itemType).toEqual(
        expectedFields,
      );
    }
  });
  ...
  it("uses only explicit, non-empty classifications with an audit note", () => {
    ...
    for (const [itemType, fields] of Object.entries(registry)) {
      for (const [field, classification] of Object.entries(fields)) {
        expect(
          classification.consumers.length,
          `${itemType}.${field}`,
        ).toBeGreaterThan(0);
        expect(
          classification.note.trim().length,
          `${itemType}.${field}`,
        ).toBeGreaterThan(0);
        expect(
  ...
  it("turns every preview/render annotation into an honest renderer disposition", () => {
    const rendererDispositions = new Set([
      "rendered",
      "meta",
      "future",
      "unsupported",
    ...
    for (const itemType of TIMELINE_DSL_ITEM_TYPES) {
      ...
      for (const [field, annotation] of Object.entries(annotations)) {
        if (
          !annotation.runtimeConsumers.some(
            (consumer: string) =>
              consumer === "preview" || consumer === "render",
          )
        ) {
          continue;
        }
        expect(
          registry[itemType]?.[field]?.consumers.some((kind) =>
            rendererDispositions.has(kind),
          ),
          `${itemType}.${field}`,
        ).toBe(true);
      }
  ...
  it("records known non-pixel fields and unsupported renderer paths instead of implying support", () => {
    ...
    expect(registry.composition?.sourcePath?.consumers).toContain(
      "unsupported",
    );
    expect(registry.composition?.spec?.consumers).toContain("unsupported");
    expect(registry.composition?.renderedAssetPath?.consumers).toContain(
      "rendered",
    );
    expect(registry["derived-overlay"]?.derivation?.consumers).toContain(
      "meta",
    );
  });
  ...
  it("keeps transition content support aligned with semantic validation", () => {
    const supportedTypes =
      (
        components as unknown as {
          TIMELINE_TRANSITION_RENDER_ITEM_TYPES?: readonly string[];
        }
  ...
  it("audits the stripped transition-content renderer separately from normal clip rendering", () => {
    ...
    expect(transitionRegistry.video?.mediaFit?.consumers).toContain(
      "unsupported",
    );
    expect(transitionRegistry.video?.properties?.consumers).toContain(
      "unsupported",
    );
    expect(transitionRegistry.video?.keyframes?.consumers).toContain(
      "unsupported",
    );
    expect(transitionRegistry.video?.mask?.consumers).toContain("rendered");
    expect(transitionRegistry.video?.effects?.consumers).toContain(
      "unsupported",
    );
    expect(transitionRegistry.image?.imageFadeIn?.consumers).toContain(
      "unsupported",
    );
    expect(transitionRegistry.text?.text?.consumers).toContain("rendered");
    expect(transitionRegistry.text?.fontWeight?.consumers).toContain(
      "rendered",
    );
    expect(transitionRegistry.text?.textAlign?.consumers).toContain(
      "unsupported",
    );
  ...
  it("classifies every shared fallback and exposes any renderer override explicitly", () => {
    const defaultCoverage =
      (
        components as unknown as {
          TIMELINE_RENDER_DEFAULT_COVERAGE?: Record<
            string,
            Record<string, { mode: string; note: string }>
          >;
        }
      ).TIMELINE_RENDER_DEFAULT_COVERAGE ?? {};
    for (const [scope, defaults] of Object.entries(TIMELINE_SHARED_DEFAULTS)) {
      expect(Object.keys(defaultCoverage[scope] ?? {}).sort(), scope).toEqual(
      ...
      for (const [field, coverage] of Object.entries(
        ...
        expect(
          ["shared", "schema-normalized", "helper", "not-read", "override"],
          `${scope}.${field}`,
        ).toContain(coverage.mode);
        expect(
          coverage.note.trim().length,
          `${scope}.${field}`,
        ).toBeGreaterThan(0);
      }
    }
    expect(Object.keys(defaultCoverage).sort()).toEqual(
      Object.keys(TIMELINE_SHARED_DEFAULTS).sort(),
    );
    expect(
      Object.values(defaultCoverage)
