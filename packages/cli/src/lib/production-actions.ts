...
export async function applyProductionMetadataAction(
  ...
  const actionPath = options.actionPath === undefined
    ? undefined
    : resolveAgentFilePathInsideCwd({
        cwd,
  ...
  const rawAction = actionPath === undefined
    ? options.action
  ...
  const metadataManifest: AssetMetadataProjectionManifest = {
    ...
    ...(actionPath === undefined ? {} : { sourceActionPath: toProjectPath(cwd, actionPath) }),
...
export async function applyProductionMetadataProjection(
  ...
  const currentMetadata = manifest.assets[assetIndex].metadata?.[metadataManifest.metadataKind] as
  ...
  const beforeMetadataHash =
    currentMetadata?.body === "cas-projection" && typeof currentMetadata.bodyHash === "string"
  ...
  const currentSourceActionHash = metadataManifest.sourceActionPath === undefined
    ? metadataManifest.sourceActionHash
