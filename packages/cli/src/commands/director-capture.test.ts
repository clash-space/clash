...
const state = {
  ...
  objects: [],
  cameras: [{ id: "camera-a", name: "A", position: [0, 1.6, 5], rotation: [0, 0, 0], fov: 50 }],
  shots: [],
...
test("captures product PNGs and commits a stage-revision readback receipt", async () => {
  const module = await import("./director").catch(() => ({} as Record<string, any>));
  ...
  const result = await module.captureDirectorStageWithReadback({
    ...
    readStage: async () => {
      reads += 1;
      return { id: "stage-a", name: "Stage A", revisionId: "revision-a", state };
    },
    render: async (input: any) => ({
  ...
  assert.equal(reads, 2, "capture must verify the Stage did not change while rendering");
  assert.equal(result.sourceStageRevisionId, "revision-a");
  assert.equal(result.verifiedStageRevisionId, "revision-a");
  assert.deepEqual(result.frames.map((frame: any) => frame.artifactId), [
    "frame-opening",
    "frame-action",
    "frame-closing",
  ]);
