...
import { attachAssetMetadata, readAssetMetadataBody } from "../lib/attach-asset-metadata";
...
import { requireAgentObservation, recordAgentObservation } from "../lib/agent-worktree-observation";
...
assetMetadataCommand
  ...
  .action(async (options) => {
    try {
      ...
      const attached: Array<Record<string, unknown>> = Object.entries(asset.metadata ?? {})
        .filter(([key, value]) =>
          key !== "metadataFills" && value && typeof value === "object" && !Array.isArray(value))
        .map(([kind, value]) => ({
...
assetMetadataCommand
  .command("set")
  .description("Attach declared metadata to an asset, storing any body out of line")
  .requiredOption("--asset <id>", "Asset id")
  .requiredOption("--kind <kind>", "Declared metadata kind")
  ...
  .action(async (options) => {
    try {
      ...
      await recordAgentObservation({
        ...
        entityId: relative(process.cwd(), result.metadataPath).split(sep).join("/"),
...
assetMetadataCommand
  .command("apply")
  .description("Apply an edited metadata projection back to the asset with CAS stale-write protection")
  .requiredOption("--file <path>", "Edited metadata projection JSON under projections/metadata/")
  // Not `--version`: Commander's global version flag intercepts it, so the
  // escape hatch would print the CLI version and exit without applying.
  .option("--expect-version <token>", "CAS token from the attach/apply that produced this projection")
  .option("--assets <path>", "Asset manifest path")
  .option("--json", "Output as JSON")
  .action(async (options) => {
    try {
      const cwd = process.cwd();
      const entityId = relative(cwd, resolve(cwd, options.file)).split(sep).join("/");
      const observed = await requireAgentObservation({
        entityKind: "asset-metadata",
        entityId,
      }).catch(() => undefined);
      const result = await applyProductionMetadataProjection({
        ...
        ...(options.assets ? { assetsPath: options.assets } : {}),
        expectedVersion: observed ?? options.expectVersion ?? "",
      });
      await recordAgentObservation({
        entityKind: "asset-metadata",
        entityId,
...
assetMetadataCommand
  .command("validate")
  .description("Check a metadata document against its declared schema without writing")
  .requiredOption("--kind <kind>", "Declared metadata kind")
  .requiredOption("--metadata <path>", "Metadata JSON path, or - for stdin")
  .option("--json", "Output as JSON")
  .action(async (options) => {
