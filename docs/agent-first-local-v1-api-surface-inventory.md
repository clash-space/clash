...
`compare` can inspect one manifest without losing provenance after verifying it
belongs to the current project's protected recovery root, matches the current
canonical replica, and does not point at out-of-set or symlinked recovery files.
Broader recovery checks and explicit import tooling are still needed.
