...
| Local room | Local-first CLI/API room surface is removed; local-api returns 404 for legacy room read/write/sync/recovery routes; cloud room remains remote/shared compatibility only; Share/Open-in-Web UI filters stale `room` readiness requirements out of cloud-sync tooltips | stale docs/tools may direct agents into removed commands, raw trace chat, or a nonexistent local room mirror requirement | keep local 404 and CLI-registration tests; design any future chat as a separate product surface |
