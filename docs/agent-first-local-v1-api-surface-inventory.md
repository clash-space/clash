...
that recovery inventory as a warning so an explicit compare/import tool can be
added without losing provenance. Broader recovery checks are still needed.
...
- `clash audit mutations --operation project_purge --entity <projectId>
  --json` reads sanitized local host mutation audit evidence without exposing
  read receipts or raw SQLite.
