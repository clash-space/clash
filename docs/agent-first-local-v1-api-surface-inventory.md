...
- `doctor storage` reports path-boundary, storage-role contract, local SQLite
  core metadata/provider auth/projection schema, and protected-store health without mutating
  project state.
...
initialized without the core metadata tables, provider auth tables, and
...
repairs the local SQLite core metadata tables, provider auth tables, plus
