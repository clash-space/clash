...
- `doctor storage` reports path-boundary, storage-role contract, local SQLite
  core metadata/projection schema, and protected-store health without mutating
  project state.
...
| Project status | CLI and local-api return roots/protected paths; `storage.canonicalReplica` includes SQLite metadata, Loro canvas replica, and protected content-addressed text/timeline revision blob roots; `doctor storage` validates path boundaries, broken asset links, stray secondary canvas replica files, protected revision content roots, existing text/timeline revision blob hash/permission integrity, existing recovery manifests, and local SQLite core metadata tables plus asset/text/timeline projection indexes; `doctor storage --repair` creates missing workspace roots, repairs the local SQLite core metadata tables plus asset/text/timeline projection indexes, and quarantines secondary canvas replica files into protected runtime recovery with manifest evidence; `doctor storage-recovery list --json` exposes quarantined recovery manifest inventory only after current-project recovery-root, destination containment, and symlink/non-regular-file checks pass, otherwise reporting invalid entries; `doctor storage-recovery compare --manifest ... --json` reports quarantined-vs-canonical file evidence without import only after current-project recovery-root, project/canonical-replica, destination containment, and symlink/non-regular-file checks pass | agents can inspect paths and host-owned repair can initialize safe workspace/schema prerequisites without blessing cwd snapshots or revision blobs as agent-writable truth | broader migration/recovery checks and explicit import tooling for quarantined replicas |
...
initialized without the core metadata tables and projection indexes needed for
projects, assets, sessions, room messages, mutation audit, agent-readable
asset references, and revision lookups. `clash doctor storage --repair` creates the standard workspace roots,
repairs the local SQLite core metadata tables plus `asset_refs`,
`asset_node_refs`, `text_revisions`, and `timeline_revisions` indexes,
...
checking manifest containment and recovery-file destinations, and `compare` can
inspect one manifest without losing provenance after verifying it belongs to the
current project's protected recovery root, matches the current canonical
replica, and does not point at out-of-set or symlinked recovery files. Broader
recovery checks and explicit import tooling are still needed.
