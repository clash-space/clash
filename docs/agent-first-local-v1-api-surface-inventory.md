...
or `updates.log` files outside the canonical Loro replica. It also warns when
local SQLite is initialized without the asset reference index schema needed for
agent-readable reference lookups. `clash doctor storage --repair` creates the standard workspace roots,
