...
| Project delete | Local API soft-deletes and CLI requires `--yes`; `clash project get --json` returns active-project `readToken`; `clash project delete --if-match` passes agent read proof; `clash project get --include-deleted --json` returns a deleted-project restore receipt; `clash project restore --if-match` requires that receipt for agent restores; v1 create/delete/restore and legacy create/update/delete responses include accepted/rejected mutation records, with legacy delete now returning JSON evidence instead of empty 204 | delayed purge, admin hard-delete, and cloud/shared recovery semantics are not specified | purge policy plus remote parity |
