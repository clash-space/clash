...
| `clash production` QA/report/receipt plan outputs | Writes agent-facing evidence under `qa/*`, `reviews/*`, `actions/*`, or reference receipt paths | Path-guarded | Use shared cwd plus realpath/symlink guard before writing; representative pipeline validation, dry-run gate, reference-role action, and text-cut action symlink escapes are rejected |
