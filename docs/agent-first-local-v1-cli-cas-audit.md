...
| `clash production` QA/report/receipt/export outputs | Writes agent-facing evidence under `qa/*`, `reviews/*`, `actions/*`, reference receipt paths, caption sidecars, timeline handoff CSV/provenance manifests, or caption-burn packages/plans | Path-guarded | Use shared cwd plus realpath/symlink guard before writing; representative pipeline validation, dry-run gate, reference-role action, text-cut action, caption export, timeline handoff export, and caption-burn sidecar symlink escapes are rejected |
