...
| `clash production` QA/report/receipt plan outputs | Writes agent-facing evidence under `qa/*`, `reviews/*`, `actions/*`, or reference receipt paths | Path-guarded | Use shared cwd plus realpath/symlink guard before writing; representative pipeline validation and dry-run gate symlink escapes are rejected |
