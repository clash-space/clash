...
- Extend first-pass local mutation audit beyond project delete/restore/purge,
  ...
  delete, asset GC delete, local-api canvas node update/delete, and local-api canvas edge delete to the remaining
  force/destructive mutation surfaces.
- Broaden direct canvas read-token fixtures beyond the live local-api node/edge
