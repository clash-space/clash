...
| `clash production apply-metadata` | Applies action metadata to an asset manifest and writes the primary metadata projection plus generic lock sidecar; JSON-derived projections also get generic lock sidecars with source metadata/action hashes | Partial | Keep the generic lock envelope and source-action hash; future editable metadata apply must use the lock/read-proof instead of blind overwrite |
...
prompt-pack, primary asset metadata, and `apply-metadata` JSON-derived
projections now also share the projection lock identity envelope through
...
- Add audit log for force/destructive mutations.
