...
| Secrets are not projection files | Provider/OAuth/API tokens stay out of agent-editable projections | Local variables endpoints are 404; provider/OAuth sensitive values are encrypted in SQLite, not projections; doctor validates and repairs only provider/OAuth table/key shape, not secret values; CLI `config.json` is file-backed but written `0600` | Mostly aligned | Keychain/token-store hardening |
...
- Local-api app tests cover explicit room sync metadata, remote-only import,
  ...
  rejection without local overwrite; conflict responses include local/remote
...
- Local-api app tests cover `sync.admission` on room message read/send responses:
  ...
  cloud-configured reads expose allowed admission.
