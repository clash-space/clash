...
- Timeline is the reference implementation for projection CAS plus first-pass
  ...
  plus primary asset metadata locks share the generic projection envelope. The
  remaining gap is richer version UI/history plus adoption by the remaining
  storyboard and derived metadata projections.
