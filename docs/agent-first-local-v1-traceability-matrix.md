...
| Room is project chat, not ACP trace | Keep room as user-visible conversation; raw traces stay private | Cloud `room_message`; CLI `room`; local-api SQLite room endpoints; local room POST returns accepted/rejected mutation records; local and cloud same project/id replays are idempotent only for identical normalized content and reject conflicting content; cloud-configured local responses mark explicit room sync `pending`; `POST /api/v1/projects/:id/room/sync` and `clash room sync` mirror local-only export plus remote-only import through the planner, return accepted `room_sync` mutation records, and reject same-id conflicts as failed/rejected mutations without overwriting either side; local ACP mention dispatch; raw ACP traces remain separate | Explicit local/remote mirror baseline implemented | Add conflict recovery UI, admission policy controls, and live room parity tests |
