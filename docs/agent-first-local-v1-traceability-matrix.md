...
| Text nodes become file-editable | Text content should be Markdown projection or text asset | `clash text pull/apply/replace`; daemon `text_cas_update` and `text_cow_replace`; generic text projection lock envelope with legacy sidecar parsing; materialized-reference apply rejection by default with explicit force; successful apply/replace creates a `clash.text.revision` milestone carrying source file path, content hash, parent revision, and actor; COW replacement records source node/content hashes and stores the applied text revision on the replacement node; local-api persists applied revision metadata in SQLite `text_revisions` through `POST /api/v1/text-revisions` and project/node lookup through `GET /api/v1/projects/:projectId/text-revisions`; CLI text apply/replace registers revisions after successful canvas mutation while remaining compatible with older hosts | Partially implemented | Add optional file-backed canonical text mode, richer UI/version history, conflict recovery, and sync mirror policy |
