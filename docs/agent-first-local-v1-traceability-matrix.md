...
| Local metadata is SQLite | Projects/assets/sessions/providers/room rows move to local SQLite | Local project, asset, session, message, agent-member, provider, OAuth, room, asset ref, asset node ref, text revision, and timeline revision rows now use `local.sqlite`; `clash doctor storage` warns when local metadata indexes are missing `asset_node_refs`, `reference_role`, `text_revisions`, `timeline_revisions`, or required indexes; `clash doctor storage --repair` ensures that asset reference and text/timeline revision index schema for old or partial SQLite files; cloud D1 schema exists | Mostly implemented | Add broader schema migration repair and sync-boundary tests |
