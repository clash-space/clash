...
| Local metadata is SQLite | Projects/assets/sessions/providers/room rows move to local SQLite | Local project, asset, session, message, agent-member, provider, OAuth, room, asset ref, asset node ref, text revision, and timeline revision rows now use `local.sqlite`; `local-metadata-store` startup upgrades old partial core metadata and asset/text/timeline projection tables before route reads or writes, and `local-provider-store` startup upgrades old partial provider/OAuth tables before provider reads, writes, or secret migration marker writes; `clash doctor storage` warns when local SQLite is missing core local metadata tables (`project`, `assets`, `asset_refs`, `runtime_session`, `agent_member`, `chat_message`, `room_message`, `mutation_audit`, `local_migration`), provider auth tables (`provider_accounts`, `provider_account_credentials`, `provider_account_supported_models`, `provider_account_model_priorities`, `provider_oauth`), or required asset/text/timeline projection columns/indexes; `clash doctor storage --repair` creates the core local metadata tables, provider auth tables, and projection indexes for old or partial SQLite files; cloud D1 schema exists | Mostly implemented | Add sync-boundary tests and broader route-level migration fixtures |
| Secrets are not projection files | Provider/OAuth/API tokens stay out of agent-editable projections | Local variables endpoints are 404; provider/OAuth sensitive values are encrypted in SQLite, not projections; doctor validates and repairs only provider/OAuth table shape, not secret values; CLI `config.json` is file-backed but written `0600` | Mostly aligned | Keychain/token-store hardening |
