...
- Text is still stored in canvas `data.content`; optional file-backed canonical
  text mode, direct visual restore/apply workflow, and local-to-cloud revision
  sync policy remain future work.
...
- Web timeline editor nodes use the same read-only revision history panel to
  ...
  `clash timeline content` recovery commands.
