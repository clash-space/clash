...
| Room | Cloud schema/routes may exist for hosted/shared collaboration; local-first CLI/API room surface is removed, legacy local room routes return 404, and Web room hook treats that 404 as hosted/cloud room unavailable instead of appending a local room log | Remote/shared compatibility only unless redesigned | Room is not raw agent trace; do not expose chat as an implicit agent state channel | Design future project chat separately from agent traces and local session history |
