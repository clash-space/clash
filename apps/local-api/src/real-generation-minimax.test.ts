...
describe.runIf(e2e.mode === "live")("MiniMax provider live recorder", () => {
  it("records selected redacted cases through the same Project backend", async () => {
    const localAccount = e2e.env.CLASH_MINIMAX_API_KEY?.trim()
      ...
      : await loadProviderLiveTestLocalAccount(e2e, {
          ...
          matches: (candidate) =>
            ...
            && (candidate.upstreamId === undefined || candidate.upstreamId === "minimax"),
        });
    const apiKey =
    ...
    const configuredRecordingPath = e2e.env.CLASH_MINIMAX_RECORDING_PATH?.trim();
    let recordingPath: string;
    if (configuredRecordingPath) {
      recordingPath = resolve(configuredRecordingPath);
      await expect(access(recordingPath)).rejects.toMatchObject({ code: "ENOENT" });
    } else {
      const root = await mkdtemp(join(tmpdir(), "clash-minimax-live-recording-"));
    ...
    if (service !== "international" && service !== "domestic") {
      throw new Error("CLASH_MINIMAX_SERVICE must be 'international' or 'domestic'");
    }
    ...
    expect(result.cases.map(({ id }) => id)).toEqual(cases.map(({ id }) => id));
    ...
    expect(new Set(requests.map((event) => event.stub.modelId)))
      .toEqual(new Set(cases.map(({ modelId }) => modelId)));
    ...
    if (selected.has("minimax-m3")) {
      expect(requests.some((event) => event.request.url.includes("/v1/chat/completions"))).toBe(true);
    }
    if (selected.has("minimax-tts")) {
      expect(requests.some((event) => event.request.url.includes("/v1/t2a_v2"))).toBe(true);
    }
    if (selected.has("minimax-music-3")) {
      expect(requests.some((event) => event.request.url.includes("/v1/music_generation"))).toBe(true);
    }
    const selectedVideoCases = cases.filter(({ modelId }) => modelId.startsWith("minimax-h3"));
    if (selectedVideoCases.length > 0) {
      expect(requests.filter((event) => event.request.url.includes("/v2/video_generation")))
        .toHaveLength(selectedVideoCases.length);
      expect(requests.filter((event) =>
        event.request.url.includes("/v2/query/video_generation/")
      ).length).toBeGreaterThanOrEqual(selectedVideoCases.length);
    }
  }, 45 * 60_000);
});
