...
function renderShellShim(commandPath: string, args: string[] = [], env: Record<string, string> = {}): string {
  return [
    "#!/bin/sh",
    "set -eu",
    ...shellEnvExports(env),
...
async function resolvePackageBin(prefixDir: string, packageSpec: string): Promise<string> {
  ...
  const candidate = join(prefixDir, "node_modules", ".bin", process.platform === "win32" ? `${binName}.cmd` : binName);
  return candidate;
}
