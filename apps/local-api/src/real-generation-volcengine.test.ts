...
import {
  ...
  selectVolcengineSeedAudioCases,
...
describe.runIf(e2e.mode === "live")("Volcengine provider live recorder", () => {
  it("records every Seed Audio input mode through the Project backend", async () => {
    ...
    if (configuredRecordingPath) {
      ...
      const root = await mkdtemp(join(tmpdir(), "clash-volcengine-live-recording-"));
      temporaryRoots.push(root);
      recordingPath = join(root, "volcengine-seed-audio-live-traffic.jsonl");
    }

    const cases = selectVolcengineSeedAudioCases(
