 ...
 * the whitelist still guards what the host can launch, while `language` decides
...
async function pluginWith(
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      runtime,
      contributes: {
        cards: [{ id: "test.lang-probe", kind: "action-card", path: "cards/lang-probe.json" }],
        functions: [{ id: "test.lang-probe", kind: "action" }],
      },
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      spec: {
        ...
        functionExportId: "test.lang-probe",
      },
    }),
    "utf8",
  );
  await writeFile(
    ...
    JSON.stringify({
      ...
      id: "lang-probe-basic",
      target: { exportId: "test.lang-probe", kind: "action" },
...
describe("declared interpreter dispatch", () => {
  ...
  it("still refuses an entrypoint shape the host cannot launch", async () => {
    const root = await pluginWith(
      {
        kind: "local",
        transport: "stdio",
