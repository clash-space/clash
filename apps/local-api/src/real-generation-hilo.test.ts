...
async function loadHiloLiveCredential(): Promise<HiloLiveCredential> {
  const accountId = e2e.env.CLASH_HILO_HUB_ACCOUNT_ID?.trim()
    || "hilo-hub-primary";
  ...
  const accessToken = await (await openPluginStore({ dataDir })).get({
    pluginId: "hrhrng.hub",
    accountId,
    key: "accessToken",
  });
...
describe.runIf(e2e.mode === "live")("Hilo Hub provider live recorder", () => {
  it("records H3 image+MP3 and Seedance audio references through the Project backend", async () => {
    ...
    if (configuredRecordingPath) {
      ...
      const root = await mkdtemp(join(tmpdir(), "clash-hilo-live-recording-"));
