  ...
  try {
    ...
    const asset = await waitForValue(async () => {
      const res = await fetch(
        `${origin}/api/v1/projects/${encodeURIComponent(projectId)}/assets/${encodeURIComponent(assetId)}`,
      );
      if (!res.ok) return null;
      return res.json();
    }, "OpenAI provider generated asset", 15000);

    assert(asset.status === "ready", "generated Project Asset is ready", asset);
    assert(typeof asset.url === "string", "generated Project Asset exposes a readable projection", asset);
    assert(asset.provenance?.model === "gpt-image-2", "generated Project Asset keeps the selected model", asset);
...
async function main() {
  ...
  const { createLocalAgentToolEnv, startLocalApiServer } = await import("../src/server.ts");
