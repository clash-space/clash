...
your projects; the desktop app, CLI, MCP server, and agents all talk to it.
...
Discovery lives at `~/.clash/run/host.json` (schema, endpoint, pid,
`pluginBrokerToken`, timestamps). Tools must read discovery instead of
...
clash canvas connect          # keeps one local Project replica connected
```

Project identity lives in `.clash/project.toml`. Collaborative state lives in
the Project's Loro CRDT replica; files in your working directory are editable
projections of it.
