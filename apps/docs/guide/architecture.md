...
described in this section is current. The metadata-probe closure limitations
listed in the Asset System guide remain deferred: this diagram shows the target
stage boundary, not proof that an inspector-unavailable Host or every L1 media
fact is already fail-closed. Team OSS, Resource Registry, multi-device
transfer, and unified Cloud/Web execution paragraphs describe the required
future Cloud adapter; existing hosted compatibility execution is not that
adapter or Asset authority.
...
transfer and reuses that id for an ambiguous retry. The target Host boundary
stages exact bytes, verifies and probes the resulting Resource, and only then
consumer-CAS publishes the authority entry. The packaged Local path supplies
the current versioned inspector; fail-closed behavior without that inspector,
hint separation, pre-probe media-type sealing, rotation normalization, and
audio-layout facts remain explicit metadata-probe gaps. Generated and
transformed outputs use the frozen `(actionRunId, outputSlot)` instead of an
import id; at-least-once producer duplicates may differ, but the consumer
retains one verified winner.
...
flowchart TB
  subgraph core["Core Asset and execution model"]
    bytes["Imported or generated bytes"] --> cas["Local Resource CAS"]
    cas --> probe["Versioned byte probe"]
    probe --> publish["Consumer CAS publication"]
...
UI/CLI submits task to local-api
  ...
  → completed outputs are staged in CAS and byte-probed into canonical metadata
