...
describe("customActionAdapter", () => {
  ...
  it("loads declared action secrets and sends model metadata to worker actions", async () => {
    ...
    await customActionAdapter.execute(ctx as any);
  ...
  it("sends bound executable plugins the shared invocation ABI without loading or exposing secrets", async () => {
    ...
    expect(JSON.parse(init.body as string)).toEqual({
      ...
      invocationId: "task-1",
      // Explicit, never inferred from an absent field: mistaking a status query for a submission
      // bills the user twice, silently, and the charge cannot be undone.
      operation: "submit",
      taskId: "task-1",
      projectId: "project-1",
      nodeId: "node-1",
  ...
  it("fails before calling the worker when a required action secret is missing", async () => {
    ...
    await expect(customActionAdapter.execute(ctx as any)).rejects.toThrow(
