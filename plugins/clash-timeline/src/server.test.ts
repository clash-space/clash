...
test("registers Timeline headless tools while the GUI is quarantined", async () => {
  ...
  assert.match(saveInputSchema.properties.state.description, /clash_timeline_schema/i);
  ...
  assert.ok(validateInputSchema.properties.document.anyOf.some(
    (variant: any) => variant.type === "object" && /clash_timeline_schema/i.test(variant.description),
  ));
  ...
  assert.match(getOutputSchema.properties.timeline.properties.state.description, /clash_timeline_schema/i);
  ...
  for (const [name, tool] of tools) {
    assert.match(tool.config.description, /^Use when: .+ Effect: .+ Returns: .+ Next: .+$/u, name);
  }
  assert.match(tools.get("clash_timeline_get")?.config.description, /stale or read-required/i);
  assert.match(tools.get("clash_timeline_save")?.config.description, /replaces the entire Timeline state/i);
  assert.match(
    tools.get("clash_timeline_save")?.config.description,
  ...
  assert.equal(read.structuredContent.contract.schemaVersion, 5);
