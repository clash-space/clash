...
var ReferenceMediaConditionSchema = z.object({
...
var ConditionalRefSpecSchema = z.object({
...
var ProviderAssetRepresentationSchema = z.enum(["provider-url", "executor-url", "bytes"]);
var ProviderAssetInputSchema = z.object({
  match: z.object({
...
var ExecutableActionCardSchema = z.object({
  ...
  for (const [index, rule] of (action.constraints ?? []).entries()) {
    if (rule.type === "mutually-exclusive") {
      rule.fields.forEach(
        (field2, fieldIndex) => validateConstraintField(field2, [
          "constraints",
          index,
          "fields",
          fieldIndex
...
var ExecutablePluginGeneratorDocumentSchema = z.object({
...
var ExecutablePluginGeneratorRegistrationSchema = z.object({
  pluginId: pluginIdSchema,
  version: z.string().trim().regex(SEMVER_PATTERN),
  schemaHash: z.string().regex(SHA256_PATTERN),
...
var ExecutablePluginBrokerResolvedReferenceSchema = z.discriminatedUnion("form", [
  z.object({
...
var ExecutableSpeechTranscriptionResultSchema = z.discriminatedUnion(
  "status",
  [
    z.object({
      status: z.literal("completed"),
...
var ExecutablePluginBrokerResponseSchema = z.discriminatedUnion(
  "status",
  [
    z.object({
...
var TimelineDiscoveryViewSchema = z.enum(
...
var TimelineTrackEnvelopeSchema = z.object(
...
var ItemUpdatesSchema = z.object(
...
var TrackUpdatesSchema = z.object(
  timelineDslAnnotatedObjectShape(TIMELINE_DSL_FIELD_ANNOTATIONS.track, {
    requiredness: "partial",
    overrides: { items: z.array(TimelineItemEnvelopeSchema) }
  })
...
var DocumentKindDefinitionSchema = z.object({
  ...
  allowedAttachmentTargets: z.array(z.enum(["project-asset", "generator-revision", "action-run"])).min(1).refine((values) => new Set(values).size === values.length, {
...
var ContentSha256Schema = z.string().regex(/^sha256:[a-f0-9]{64}$/u);
var NonEmptyIdSchema = z.string().trim().min(1).max(500);
...
var WorkspacePortableJsonObjectSchema = z.record(z.unknown()).superRefine((root, context) => {
...
var WorkspaceBundleFileRoleSchema = z.enum([
  "workspace",
  "project",
  "object"
]);
var WorkspaceBundleFileSchema = z.object({
...
var WorkspaceBundleResourceSchema = z.object({
  resource: ResourceSchema,
  path: WorkspaceBundleRelativePathSchema
}).strict();
var WorkspaceBundleDocumentBodySchema = z.object({
  contentHash: ContentSha256Schema,
  byteLength: z.number().int().nonnegative(),
  contentType: z.literal("application/json"),
  path: WorkspaceBundleRelativePathSchema
}).strict();
var WorkspaceBundleTextRevisionSchema = z.object({
...
var WorkspaceBundleProjectSchema = z.object({
  path: z.literal(WORKSPACE_BUNDLE_PROJECT_PATH),
  codec: z.literal("loro-shallow-snapshot"),
  codecVersion: z.literal(1)
}).strict();
var WorkspaceSemanticModelReferenceSchema = z.object({
  modelId: NonEmptyIdSchema
}).strict();
var WorkspaceBundleExcludedPathSchema = z.object({
  path: WorkspaceBundleRelativePathSchema,
  reason: z.enum([
    "vcs-private",
...
var WorkspaceBundleSourceSchema = z.object({
...
var WorkspaceBundleContentSchema = z.object({
...
var WorkspaceBundleManifestSchema = z.object({
  ...
  const issue2 = (path, message) => context.addIssue({ code: z.ZodIssueCode.custom, path, message });
  ...
  manifest.files.forEach((file, index) => {
    ...
    if (file.role === "workspace" && !file.path.startsWith(`${manifest.content.workspaceRoot}/`)) {
      issue2(
        ["files", index, "path"],
  ...
  manifest.content.resources.forEach((entry, index) => {
    ...
    if (file?.role !== "object" || file.sha256 !== digest || file.bytes !== entry.resource.byteLength) {
      issue2(
        ["content", "resources", index, "path"],
  ...
  manifest.content.documentBodies.forEach((entry, index) => {
    ...
    const file = filesByPath.get(entry.path);
    if (file?.role !== "object" || file.sha256 !== digest || file.bytes !== entry.byteLength) {
      issue2(
        ["content", "documentBodies", index, "path"],
  ...
  manifest.content.textRevisions.forEach((entry, index) => {
    ...
    const file = filesByPath.get(entry.path);
    if (file?.role === "object" && !file.sha256.startsWith(entry.revision.contentHash)) {
      issue2(
        ["content", "textRevisions", index, "revision", "contentHash"],
        "Text revision contentHash must match the exported body SHA-256 prefix"
      );
    }
    if (file?.role === "object" && entry.path !== `objects/sha256/${file.sha256}`) {
      issue2(
        ["content", "textRevisions", index, "path"],
  ...
  manifest.files.forEach((file, index) => {
    const referenced = file.role === "workspace" || file.role === "project" && file.path === manifest.content.project.path || file.role === "object" && (resourcePaths.has(file.path) || documentPaths.has(file.path) || textRevisionPaths.has(file.path));
    if (!referenced) {
      issue2(
        ["files", index, "path"],
...
var WorkspaceTransferTimestampSchema = z.string().datetime({ offset: true });
var WorkspaceTransferCapabilitySchema = z.string().min(16).max(500).regex(/^[A-Za-z0-9_-]+$/u);
var WorkspaceTransferIdempotencyKeySchema = z.string().trim().min(1).max(500);
...
var WorkspaceImportStartSchema = z.object({
  schemaVersion: z.literal(1),
...
var WorkspaceImportSessionSchema = z.object({
  schemaVersion: z.literal(1),
...
var WorkspaceImportFileUploadReceiptSchema = z.object({
  schemaVersion: z.literal(1),
